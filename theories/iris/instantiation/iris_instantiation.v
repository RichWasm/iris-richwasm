  ...
  Forall (fun '{| moddata_data := (Mk_memidx n);
            ...
            | Some doffi =>
              ...
              | Some (Mk_memidx k) =>
                ...
                | Some mi => (N.of_nat (nat_of_int doffi + length bs) <= length_mem mi)%N
                | None => False
                end
              | _ => 
...
Definition import_resources_wasm_domcheck (v_imps: list module_export) (wfs: gmap N function_closure) (wts: gmap N tableinst) (wms: gmap N memory) (wgs: gmap N global) : iProp Σ :=
  ⌜ dom (D := gset N) wfs ≡ list_to_set (fmap N.of_nat (ext_func_addrs (fmap modexp_desc v_imps))) /\
    dom (D := gset N) wts ≡ list_to_set (fmap N.of_nat (ext_tab_addrs (fmap modexp_desc v_imps))) /\
    dom (D := gset N) wms ≡ list_to_set (fmap N.of_nat (ext_mem_addrs (fmap modexp_desc v_imps))) /\
    dom (D := gset N) wgs ≡ list_to_set (fmap N.of_nat (ext_glob_addrs (fmap modexp_desc v_imps))) ⌝.
...
Definition tab_domcheck v_imps (wts: gmap N tableinst) : Prop :=
  dom (D := gset N) wts ≡ list_to_set (N.of_nat <$> (ext_tab_addrs (fmap modexp_desc v_imps))).

Definition mem_domcheck v_imps (wms: gmap N memory) : Prop :=
  dom (D := gset N) wms ≡ list_to_set (N.of_nat <$> (ext_mem_addrs (fmap modexp_desc v_imps))).

Definition glob_domcheck v_imps (wgs: gmap N global) : Prop :=
  dom (D := gset N) wgs ≡ list_to_set (N.of_nat <$> (ext_glob_addrs (fmap modexp_desc v_imps))).
...
Proof.
  ...
  assert (i ∈ dom (D := gset N) wts) as Hidom.
...
Proof.
  ...
  assert (i ∈ dom (D := gset N) wms) as Hidom.
  { by apply elem_of_dom. }
...
Proof.
  ...
  replace (length_mem mem) with (length_mem m0) => //.
...
Proof.
  ...
  { apply map_disjoint_list_to_map_l.
    ...
    rewrite length_fmap in Hcontra.
    by lias.
  }
  { apply map_disjoint_list_to_map_l.
    rewrite List.Forall_forall.
  ...
  - simpl.
    ...
    iMod (pointsto_persist with "Htsizemapsnew") as "Htsizemapsnew".
    iMod (pointsto_persist with "Htlimmapsnew") as "Htlimmapsnew".
    iModIntro.
...
Lemma alloc_mem_state_update mems mems' nm:
  ...
  gen_heap_interp (gmap_of_list (fmap length_mem mems)) -∗
...
Proof.
  ...
  (* table size *)
  specialize (gmap_of_list_append_big (length_mem <$> mems) (length_mem <$> nm)) as Hgmap.
  destruct Hgmap as [Hgunion Hgdisj].
  rewrite fmap_app.
  rewrite Hgunion.
  ...
  { apply map_disjoint_list_to_map_l.
    ...
    repeat rewrite length_fmap in Hcontra.
    by lias.
  }
  { apply map_disjoint_list_to_map_l.
    rewrite List.Forall_forall.
...
Proof.
  ...
  - assert (off+1 <=? length (table_data tab)) as Hle2.
    { apply PeanoNat.Nat.leb_le.
      ...
      by lias. }
    by rewrite Hle2. 
  - rewrite length_app length_take => /=; rewrite length_drop.
    ...
    do 3 f_equal.
    rewrite <- drop_drop.
    rewrite drop_app_length'; last first.
    { rewrite length_app length_take => /=.
      by lias.
    }
    rewrite drop_drop.
...
Lemma update_wts_dom_preserve wts wts' inst e_offs elems:
  ...
  dom (D := gset N) wts ≡ dom (D := gset N) wts'.
Proof.
  ...
  - move: H.
    ...
    fold update_wts.
    move => Hwtsupd.
    assert (dom (D := gset N) wts ≡ (dom (D := gset N) (<[N.of_nat n0 := t0]> wts))) as Hdomeq; first by rewrite dom_insert_lookup => //.
    rewrite Hdomeq.
    by rewrite IHe_offs => //.
Qed.
...
Proof.
  ...
  unfold length_mem, memory_list.length_mem in Hmemlength.
  ...
  replace (length_mem <$> s_mems ws') with (length_mem <$> s_mems ws); last first.
  {
    rewrite Hws' => /=.
    rewrite list_fmap_insert => /=.
    rewrite list_insert_id => //.
...
Lemma init_mem_state_update (ws ws': store_record) (inst: instance) (d_off: N) (d: module_data) (m_ind: nat) (mem: memory) (d_pay: list byte) mem' :
  ...
  (d_off + N.of_nat (length d.(moddata_init)) <= length_mem mem)%N ->
...
Proof.
  ...
  unfold length_mem, memory_list.length_mem in Heboundcheck.
  rewrite -> Nat2N.id in *.
  rewrite <- Htind in Hinitmem.
  destruct d, moddata_data; simpl in *.
  rewrite length_map in Hinitmem.
...
Lemma update_wms_dom_preserve wms wms' inst d_offs datas:
  ...
  dom (D := gset N) wms ≡ dom (D := gset N) wms'.
Proof.
  ...
  - move: H.
    ...
    fold update_wms.
    move => Hwmsupd.
    assert (dom (D := gset N) wms ≡ (dom (D := gset N) (<[N.of_nat n0 := m0]> wms))) as Hdomeq; first by rewrite dom_insert_lookup => //.
    rewrite Hdomeq.
    by rewrite IHd_offs => //.
Qed.
  ...
  - move: H.
    ...
    + destruct (update_mem _ _ _) eqn:Hupdmem => //=.
      ...
      { rewrite H0 in Hwmslookup.
        ...
        rewrite length_map.
        rewrite length_take length_drop.
        unfold length_mem, memory_list.length_mem in H1.
        simpl in *.
        apply/andP; split => //.
...
Definition wms_bound_check (wms: gmap N memory) inst (d_offs : list N) datas :=
  all2 (fun off e => match inst.(inst_memory) !! (match e.(moddata_data) with | Mk_memidx i => i end) with
                  | Some i => match wms !! (N.of_nat i) with
                             | Some mi => (off + N.of_nat (length e.(moddata_init)) <=? length_mem mi)%N
                             | None => false
                             end
                  | None => false
                  end)
...
Proof.
  ...
  assert (map_disjoint wts wtsalloc) as Hwtsdisj.
  {
    ...
    assert (i ∈ dom (D := gset N) wts) as Hidom; first by apply elem_of_dom.
    ...
    { apply lookup_lt_Some in Hrlookup.
      by rewrite length_repeat in Hrlookup. }
    ...
    (mod_elem m) = Some wts') as Hwtsupd.
  {
    ...
    rewrite nth_error_lookup in H6.
    destruct ( (s_tables0 ++
          ((λ '{| tt_limits := {| lim_min := min; lim_max := maxo |} |},
              {|
                table_data := repeat None (ssrnat.nat_of_bin min); table_max_opt := maxo
              |}) <$> map modtab_type (mod_tables m))) !! t0) eqn:Hnl => //.
    all: rewrite Hnl in H6 => //. 
    ...
    { (* Alloc *)
      ...
      assert (wts !! (N.of_nat t0) = None) as Hwtslookup.
      {
  ...
  iAssert ( ([∗ map] n ↦ tabinst ∈ wts', n↦[wtblock] tabinst) -∗ (import_tab_wasm_check v_imps t_imps (module_import_init_tabs m inst wts)) ∗ (module_inst_resources_tab (module_inst_build_tables m inst) (drop (get_import_table_count m) inst.(inst_tab))))%I as "Htabsplit".
  {
    ...
    { 
      ...
      (* replace *)
      {
        ...
        destruct (nth_error _ n0) eqn: Hwtlookup => //=.
        ...
        { destruct (update_tab _ _ _) eqn: Hupdtab => //=.
          ...
          { apply Forall_lookup.
            ...
            destruct (ext_tabs _ !! n1) eqn:Hexttabslookup => //.
            destruct t3 => /=.
            destruct (wts !! N.of_nat n2) eqn:Hwtslookup2 => //.
          ...
          assert (N.of_nat n0 ∈ (dom (D := gset N) wts)) as Ht0dom; first by apply elem_of_dom.
          ...
          { f_equal.
            ...
            remember ((take (nat_of_int t) (table_data t1) ++
     ((λ '(Mk_funcidx fidx), inst_funcs !! fidx) <$> modelem_init) ++
     drop (nat_of_int t + length modelem_init) (table_data t1))) as l.
            replace (length (table_data t1)) with (length l); first by rewrite firstn_all.
            subst.
            repeat rewrite length_app.
            rewrite length_fmap length_take length_drop.
            apply PeanoNat.Nat.leb_le in Hle.
            assert (nat_of_int t <= length (table_data t1)) as Hlt.
            { unfold nat_of_int; by lias. }
            replace (nat_of_int t `min` length (table_data t1)) with (nat_of_int t); last by lias.
            rewrite length_map in Hle.
  ...
  length (module_inst_build_tables m inst)) as Htblen.
      ...
      { rewrite Httlen Hvttablen.
        ...
        rewrite drop_app_length'; last by rewrite length_fmap.
        rewrite map_fmap in Htindex.
        rewrite Htindex.
      ...
      { rewrite gen_index_length.
        apply fold_left_preserve => //.
        { rewrite Heqtablebase.
          by rewrite length_fmap length_map.
        }
        move => x act Hlen.
        destruct act, modelem_table.
        destruct (n <? _) => //.
      ...
      rewrite length_fmap in Hinsttab.
      clear H1.
      rewrite -> Hinsttab in *.
      clear Heqtablebase.
      rewrite -> Htapp in *.
      ...
      destruct ((s_tables0 ++ tablebase) !! n0) eqn:Htl => //=.
      eapply IHmod_elem => //.
      { apply all2_Forall2.
        apply all2_Forall2 in Helemcb.
        apply Forall2_same_length_lookup.
        ...
        rewrite lookup_app.
        destruct (s_tables0 !! t1) eqn:Hstlookup => /=.
        ...
        { rewrite Hstlookup in H4.
          ...
          destruct (decide ((n - length (ext_t_tabs t_imps)) = (t1 - length s_tables0))) eqn:Hd => //=.
          { rewrite e.
            ...
            rewrite length_take.
            repeat rewrite length_app.
            rewrite length_take length_drop length_fmap.
            apply N.leb_le.
      ...
      }
      {
        ...
        { (* Updating the table *)
          ...
          f_equal.
          rewrite length_fmap.
          destruct (n<? length(ext_tabs (modexp_desc <$> v_imps))) eqn:Hlt; first by move/Nat.ltb_lt in Hlt; apply lookup_ge_None in Himplookup; rewrite length_fmap in Himplookup; lias.
          rewrite Heqtablebase'.
          ...
          { unfold update_tab in Hupdtab.
            ...
            repeat rewrite length_fmap.
            replace (Z.to_nat (Wasm_int.Int32.intval t)) with (nat_of_int t) => //.
            assert (length (take (nat_of_int t) (table_data t1) ++
   ((λ '(Mk_funcidx j), inst_funcs !! j) <$> modelem_init) ++
   drop (nat_of_int t + length modelem_init) (table_data t1)) = length (table_data t1)) as Hlen.
            { repeat rewrite length_app.
              rewrite length_take length_drop length_fmap.
              move/Nat.leb_le in Hle.
              apply N.leb_le in Hcb.
              rewrite lookup_app_r in Htl; last by lias.
              rewrite length_fmap in Htl.
  ...
  assert (d_offs = d_inits') as Hdeq.
  {
    ...
    { replace (length d_offs) with (length (mod_data m)); last by apply Forall2_length in Hinstdata.
      replace (length (mod_data m)) with (length (moddata_offset <$> mod_data m)); last by rewrite length_fmap.
      rewrite Hmoddata.
      by rewrite length_fmap.
    }
  ...
  assert (map_disjoint wms wmsalloc) as Hwmsdisj.
  {
    ...
    assert (i ∈ dom (D := gset N) wms) as Hidom; first by apply elem_of_dom.
    ...
    (mod_data m) = Some wms') as Hwmsupd.
  {
    ...
    rewrite nth_error_lookup in H5.
    destruct ((s_mems1 ++ module_inst_mem_base (mod_mems m)) !! m0) eqn:Hnl; rewrite Hnl in H5 => //.
    rewrite lookup_app in Hnl.
    rewrite -> Hinstm in *.
    ...
    { (* Imports *)
      ...
      destruct (wms !! N.of_nat m0) eqn:Hwmslookup => //.
      rewrite lookup_union_l'; last done.
      rewrite Hwmslookup => //.
      apply N.leb_le.
    ...
    { (* Alloc *)
      assert (wms !! (N.of_nat m0) = None) as Hwmslookup.
      {
        simpl in Hinstmem.
        apply gen_index_lookup_Some in Hinstmem as [-> Hlt].
        rewrite length_fmap in Hlt.
        apply not_elem_of_dom.
        rewrite Hmdom.
        rewrite elem_of_list_to_set.
        rewrite elem_of_list_fmap.
  ...
  { (* bound check *)
    ...
    - inversion Hdatacb; subst.
      ...
      { (* Imported *)
        ...
        specialize (Himpwasm k m e Hvlookup Hmlookup) as Hmdesc.
        destruct m0; inversion Hlookup; subst; clear Hlookup.
        simpl in *.
        rewrite H2 in Hmdesc.
        ...
        apply N.leb_le.
        unfold length_mem, memory_list.length_mem.
        unfold instantiation.length_mem, memory_list.length_mem in H1.
        simpl in *.
        by lias.
      }
      { (* Alloc *)
      ...
      (* replace *)
      {
        ...
        { destruct (update_mem _ _ _) eqn: Hupdmem => //=.
          ...
          { apply Forall_lookup.
            ...
            destruct (ext_mems _ !! n1) eqn:Hextmemslookup => //.
            destruct m1 => /=.
          ...
          rewrite Hwmslookup.
          destruct inst; simpl in *; subst.
          assert (N.of_nat n0 ∈ (dom (D := gset N) wms)) as Hm0dom; first by apply elem_of_dom.
          ...
          { f_equal.
            ...
            rewrite length_fmap in Hupdmem.
            destruct (_+_<=? _) eqn:Hle => //=.
            inversion Hupdmem; subst; clear Hupdmem.
            simpl in *.
            unfold assert_const1_i32_to_nat => /=.
            ...
            remember (_ ++ _ ++ _) as l.
            replace (length (ml_data (mem_data m))) with (length l); first by rewrite firstn_all.
            subst.
            repeat rewrite length_app.
            rewrite length_fmap length_take length_drop.
  ...
  length (module_inst_build_mems m inst)) as Hmblen.
      ...
      { rewrite Hmtlen Hvtmemlen.
        rewrite Hinstm.
        by rewrite drop_app_length'; last by rewrite length_fmap.
      }      
      iApply big_sepL2_big_sepM => //.
      { by apply gen_index_NoDup. }
      Unshelve.
      ...
      destruct (_ !! n) eqn:Hnl => //=.
      repeat rewrite -> nth_error_lookup in *.
      destruct ((s_mems1 ++ memorybase) !! n0) eqn:Hml => //=. 
      eapply IHmod_data => //.
      { apply all2_Forall2.
        apply all2_Forall2 in Hdatacb.
        apply Forall2_same_length_lookup.
        ...
        rewrite lookup_app in H4.
        rewrite lookup_app.
        destruct (s_mems1 !! m0) eqn:Hstlookup => /=.
        { rewrite Hstlookup in H4.
          by rewrite Hstlookup.
        }
        { rewrite Hstlookup in H4.
          rewrite Hstlookup.
          destruct (n <? _) eqn:Hlt => //=.
          destruct (memorybase !! (m0 - _)) eqn:Hmblookup => //=.
          destruct (decide ((n - length (ext_t_mems t_imps)) = (m0 - length s_mems1))) eqn:Hd => //=.
          { rewrite e.
            ...
            simpl in *.
            apply N.leb_le in H4.
            unfold instantiation.length_mem, memory_list.length_mem in H4.
            by lias.
          }
          {
      ...
      }
      {
        ...
        { (* Updating the mem *)
          ...
          { unfold update_mem in Hupdmem.
            ...
            rewrite -> Ht in *.
            assert (length (take (nat_of_int t) (ml_data (mem_data m0)) ++
   (compcert_byte_of_byte <$> moddata_init) ++
   drop (nat_of_int t + length moddata_init) (ml_data (mem_data m0))) = length (ml_data (mem_data m0))) as Hlen.
            { repeat rewrite length_app.
              rewrite length_take length_drop length_fmap.
              move/Nat.leb_le in Hle.
              apply N.leb_le in Hcb.
              rewrite length_fmap in Hml.
              destruct (s_mems1 !! _) eqn:Hsmlookup; first by apply lookup_lt_Some in Hsmlookup; lias.
  ...
  iSplitL "Hfmapsto".
  {
    ...
    rewrite drop_all.  rewrite length_map.
    rewrite Nat.sub_diag. simpl. done.
    rewrite drop_all. rewrite length_map. rewrite Nat.sub_diag.
    unfold drop. simpl. 
    unfold gen_index.
    rewrite length_app repeat_app imap_app fmap_app => /=.
    rewrite length_repeat.
    rewrite big_sepL_app.
  ...
  (* Globals *)
  {
    ...
    iIntros "%Hginitslen".
    rewrite length_app in Hginitslen.
    ...
    rewrite module_inst_global_init_cat; last by rewrite length_fmap; lias.
    ...
    iDestruct "Ht" as "(Ht & _)".
    iSplit => //.
    rewrite length_fmap length_combine.
