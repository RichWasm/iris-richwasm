...
From RichWasm.wasm Require Export datatypes operations properties instantiation.
...
| HR_host_step:
  forall s (vis: vi_store) m (viexps: list vi) vm vimps imps imp_descs s' vis' ms idecs' inst (exps: list module_export) start vs f fs,
    ...
    host_reduce Run s vis ms ((ID_instantiate viexps vm vimps) :: idecs') fs f vs Run s' vis' ms idecs' fs f (map_start start)
| HR_host_step_init_oob: forall s (vis: vi_store) m (viexps: list vi) vm vimps imps imp_descs ms idecs' (exps: list module_export) f vs fs,
    ...
    host_reduce Run s vis ms ((ID_instantiate viexps vm vimps) :: idecs') fs f vs Crash s vis ms idecs' fs f [AI_trap]
 | HR_call_host_action :
  forall (s:store_record) vis ms idecs (s':store_record) (tf:function_type)
    ...
    llfill lh res = LI' ->
    host_reduce Run s vis ms idecs fs f0 LI Run s' vis ms idecs fs f0 LI'
...
| HR_get_global: ∀ s f vis ms g v_glob fs vs,
    ...
    const_list vs ->
    host_reduce Run s vis ms [H_get_global g] fs f vs Run s vis ms [] fs f (AI_basic (BI_const (g_val v_glob)) :: vs)
| HR_trap: ∀ s vis ms g fs f,
    host_reduce Run s vis ms [H_get_global g] fs f [::AI_trap] Bail s vis ms [] fs f [::AI_trap]
...
Lemma call_host_reduce_det o s vis ms idecs fs tf h vcs o1 s1 vis1 ms1 idecs1 fs1 f1 es1 o2 s2 vis2 ms2 idecs2 fs2 f2 es2 f0 llh LI :
  llfill llh [AI_call_host tf h vcs] = LI ->
  host_reduce o s vis ms idecs fs f0 LI o1 s1 vis1 ms1 idecs1 fs1 f1 es1 ->
  host_reduce o s vis ms idecs fs f0 LI o2 s2 vis2 ms2 idecs2 fs2 f2 es2 ->
  (o1, s1, vis1, ms1, idecs1, fs1, f1, es1) = (o2, s2, vis2, ms2, idecs2, fs2, f2, es2).
Proof.
...
Lemma get_global_host_det he o s vis ms fs f0 vs o1 s1 vis1 ms1 idecs1 fs1 f1 vs1 vs2 o2 s2 vis2 ms2 idecs2 fs2 f2 :
  const_list vs ->
  host_reduce o s vis ms [H_get_global he] fs f0 vs o1 s1 vis1 ms1 idecs1 fs1 f1 vs1 ->
  host_reduce o s vis ms [H_get_global he] fs f0 vs o2 s2 vis2 ms2 idecs2 fs2 f2 vs2 ->
  (o1, s1, vis1, ms1, idecs1, fs1, f1, vs1) = (o2, s2, vis2, ms2, idecs2, fs2, f2, vs2).
Proof.
...
Lemma trap_host_det 
  g o s vis ms fs f0 
  o1 s1 vis1 ms1 idecs1 fs1 f1 vs1 vs2 
  o2 s2 vis2 ms2 idecs2 fs2 f2 :
  host_reduce o s vis ms [g] fs f0 [AI_trap] o1 s1 vis1 ms1 idecs1 fs1 f1 vs1 ->
  host_reduce o s vis ms [g] fs f0 [AI_trap] o2 s2 vis2 ms2 idecs2 fs2 f2 vs2 ->
  (o1, s1, vis1, ms1, idecs1, fs1, f1, vs1) = (o2, s2, vis2, ms2, idecs2, fs2, f2, vs2).
Proof.
  intros Hred1 Hred2.
...
Definition state : Type := obs * store_record * vi_store * (list module) * (list host_action ) * frame.
...
Definition prim_step (e : host_expr) (s : state) (os : list observation) (e' : host_expr) (s' : state) (fork_es' : list host_expr) : Prop :=
  let '(o, ws, vis, ms, fs, f) := s in
  let '(o', ws', vis', ms', fs', f') := s' in
  let '(hes, wes) := e in
  let '(hes', wes') := e' in
  host_reduce o ws vis ms hes fs f wes o' ws' vis' ms' hes' fs' f' wes' /\ os = [] /\ fork_es' = [].
...
Proof.
  rewrite /prim_step.
  move => [hes wes] [[[[[o ws] vis] hprog] fs] f] κ [hes' wes'] [[[[[o' ws'] vis'] hprog'] fs'] f'] efs [HRed _].
  ...
  state_interp σ _ κs _  :=
    let: (o, s, vis, ms, fs, f) := σ in
     ((gen_heap_interp (gmap_of_list s.(s_funcs))) ∗
      ...
      (ghost_map_auth obsGName 1 (<[ tt := o ]> ∅))
    )%I;
...
Lemma wp_call_host_action_no_state_change s E hes tf h hi f vcs (Φ : host_val -> iProp Σ) res llh LI LI' :
  ...
  N.of_nat hi ↦[ha] f ∗
  ↪[RUN] ∗
  ▷ (N.of_nat hi ↦[ha] f -∗ ↪[RUN] -∗ WP ((hes, LI') : host_expr) @ s ; E {{ v, Φ v }})
  ⊢ WP ((hes, LI) : host_expr) @ s ; E {{ v, Φ v }}.
Proof.
  iIntros (Hh HLI HLI' Hexec) "(Hhi & Hrun & Hwp)".
  ...
  - iIntros (σ ns κ κs nt) "Hσ".
    ...
    iIntros "Hfupd".
    destruct σ as [[[[[o ws] vi] ms] has] f0].
    iDestruct "Hσ" as "(? & ? & ? & ? & ? & ? &  Hha & ? & ? & ? & ? & ? & Hobs)".
    iCombine "Hobs" "Hrun" gives "%Heq".
    vm_compute in Heq; inversion Heq; subst o.
  ...
  - iIntros "!>" (es σ2 efs HStep).
    destruct σ2 as [[[[[o2 s2] vi2] ms2] has2] f2].
...
Proof.
  ...
  - iIntros "!>" (es σ2 efs HStep).
    ...
    apply Nat.ltb_lt in Habs. lia. done. 
    ...
    instantiate (1 := t).
    rewrite Z_N_nat. iIntros "Hpound". done.
    simpl in Hlt. done.
  ...
  assert ((instantiate ws m (fmap modexp_desc v_imps) ((ws_res, inst_res, v_exps), None))) as Hinst.
  {
    ...
    - (* global initializers *)
      ...
      + destruct (g_inits !! i) eqn:Hginit => /=; last by apply lookup_lt_Some in Hmglob; apply lookup_ge_None in Hginit; lias.
        constructor.
        apply fmap_fmap_lookup with (i := i) in Hmodglob.
    ...
    - (* table initializers *)
      ...
      + destruct (e_inits !! i) eqn: Heinit => /=; last by apply lookup_lt_Some in Hmelem; apply lookup_ge_None in Heinit; lias.
        rewrite Heinit.
        constructor.
        apply fmap_fmap_lookup with (i := i) in Hmodelem.
    ...
    - (* memory initializers *)
      ...
      + destruct (d_inits !! i) eqn: Hdinit => /=; last by apply lookup_lt_Some in Hmdata; apply lookup_ge_None in Hdinit; lias.
        rewrite Hdinit.
        constructor.
        apply fmap_fmap_lookup with (i := i) in Hmoddata.
