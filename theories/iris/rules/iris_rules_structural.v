...
Lemma wp_frame_value (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) es n (f f0: frame) vs :
  iris.to_val es = Some (immV vs) ->
  length es = n ->
  ↪[frame] f0 -∗ ↪[RUN] -∗
  ▷ (Φ (immV vs)) -∗
  WP es @ s; E FRAME n; f {{ v, (Φ v ∗ ↪[RUN]) ∗ ↪[frame] f0 }}.
Proof.
  iIntros (Hv Hlen) "Hframe Hrun H".
  rewrite wp_frame_rewrite.
  apply to_val_const_list in Hv as Hconst.
  iApply (wp_lift_atomic_step with "[-]"); simpl ; trivial;eauto.
  unfold iris.to_val => /=.
...
(*    destruct σ as [[ ws locs] inst]. *)
  ...
  - (* destruct σ as [[ws locs] inst]. *)
    ...
    iris_wp_def.only_one_reduction H. all:simplify_eq;iFrame. rewrite Hv. iFrame.
...
{ iIntros (Hnone) "[Htrap [Hf0 [Hes1 Hes2]]]".
  ...
  { repeat rewrite wp_unfold /wp_pre /=. rewrite Hetov.
    ...
    iIntros (σ ns κ κs nt) "Hσ".
    destruct σ as [[[obs s1] locs] inst].
    ...
    { iMod "Hes1" as "[HPsi Hf]".
      ...
      iSpecialize ("Hes2" $! (_,s1,locs,inst) ns κ κs nt with "[$]").
      iFrame. }

    

    iSpecialize ("Hes1" $! (_,s1,f_locs,f_inst) ns κ κs nt with "[$]").
    iMod "Hes1" as "[%H1 H2]".
    iModIntro.
    iSplit.
    { iPureIntro.
    ...
    iIntros (e2 σ2 efs HStep').
    destruct σ2 as [[[obs3 s3] locs2] inst2].
    ...
    { apply prim_step_obs_efs_empty in HStep'' as Hemp. inversion Hemp;subst;clear Hemp.
      iSpecialize ("H2" $! e' (_,s3,v'',i'') [] HStep'').
      iMod "H2".
      repeat iModIntro.
      repeat iMod "H2".
      iDestruct "H2" as "(Hσ & H)".
