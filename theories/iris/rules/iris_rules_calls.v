...
Context `{!wasmG Σ}.
  ...
  Proof.
    ...
    set (σ := (_,s0,l,i0)).
    eassert (reduce _ s0 {| f_locs := l; f_inst := i0 |}
           (ves ++ [AI_invoke a])%list _ s0 {| f_locs := l; f_inst := i0 |}
  ...
  Lemma wp_invoke_host s E ves vcs n t1s t2s a h (f: frame) Φ :
    ves = v_to_e_list vcs ->
    length vcs = n ->
    length t1s = n ->
...
Proof.
  iIntros (Hves Hvcs Ht1s) "Ha Hf Hrun Hwp".
  iApply wp_lift_step => //=.
  - unfold iris.to_val.
    rewrite map_app => //=.
    rewrite merge_app.
...
Qed.
  ...
  Lemma wp_call_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) f0 (i : nat) a j lh :
    ...
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [AI_invoke a] @ s; E CTX j; lh {{ v, Φ v }}) -∗
     WP [AI_basic (BI_call i)] @ s; E CTX j; lh {{ v, Φ v }}.
  Proof.
    iIntros (Hfuncs) "Hf Hrun HΦ".
    iIntros (LI Hfill).
  ...
  Lemma wp_call (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) f0 (i : nat) a :
    (inst_funcs (f_inst f0)) !! i = Some a -> 
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [AI_invoke a] @ s; E {{ v, Φ v }}) -∗
  ...
  Lemma wp_call_indirect_success_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) (f0 : frame) (i j : immediate) a c cl d lh :
    ...
    ▷ ((N.of_nat j) ↦[wt][N.of_nat (Wasm_int.nat_of_uint i32m c)] (Some a)
       ∗ (N.of_nat a) ↦[wf] cl
       ∗ ↪[frame] f0 -∗ ↪[RUN] -∗ WP [AI_invoke a] @ s; E CTX d; lh {{ v, Φ v }}) -∗
  ...
  inversion Hrun; subst o.
    ...
    - iApply fupd_mask_intro;[solve_ndisj|].
      ...
      destruct HStep as (H & -> & ->).
      assert (reduce Run s0 {| f_locs := l; f_inst := i0 |} LI Run s0 {| f_locs := l; f_inst := i0 |} LI') as Hred'.
      { eapply r_label;eauto. }
      eapply reduce_det in H as HH;[|apply Hred'].
  ...
  Proof.
    ...
    iApply wp_lift_atomic_step;[auto|].
    iIntros ([[ [??] ?] ?] ns κ κs nt) "(Hσ1&Hσ2&Hσ3&Hσ4&Hσ5&Hσ6&Hσ7&Hσ8&Hσ9&Hobs)".
     iDestruct (ghost_map_lookup with "Hobs Hrun") as "%Hrun".
  rewrite lookup_insert in Hrun.
  inversion Hrun; subst o.
    iApply fupd_frame_l.
