...
Section trap_rules.
  ...
  Lemma wp_trap (s : stuckness) (E : coPset) (Φ : iris.val -> iProp Σ) (vs1 es2 : iris.expr) (f: frame) :
    const_list vs1 ->
    Φ trapV -∗
    ↪[frame] f -∗
  ...
  Lemma wp_trap_frame (s : stuckness) (E : coPset) (Φ : frame -> iris.val -> iProp Σ) (vs1 es2 : iris.expr) (f: frame) :
    const_list vs1 ->
    Φ f trapV -∗
    ↪[frame] f -∗
    WP vs1 ++ [AI_trap] ++ es2 @ s; E {{ v, Φ f v ∗ ↪[frame] f }}.
  Proof.
    iLöb as "IH" forall (s E vs1 es2 f). 
    iIntros (Hconst) "HΦ Hf".
    destruct (iris.to_val (vs1 ++ [AI_trap] ++ es2)) eqn:Hsome.
  ...
  Lemma wp_seq_trap (s : stuckness) (E : coPset) (es1 es2 : language.expr wasm_lang) (f f': frame) :
    ↪[frame] f ∗
  ...
  Lemma wp_val_app_trap (s : stuckness) (E : coPset) vs v' (es : language.expr wasm_lang) (f f': frame) :
  ...
  Lemma wp_frame_trap (s : stuckness) (E : coPset) (Φ : iris.val -> iProp Σ) n (f f0: frame) :
    ↪[frame] f0 -∗
     ▷ (Φ trapV) -∗
  ...
  Lemma wp_frame_trap_nested (s : stuckness) (E : coPset) n (f f0: frame) LI (f':frame) :
  ...
  forall (s E LI f f' f0 Hnone).
    ...
    { iApply wp_unfold. unfold wp_pre. simpl. rewrite Hnone.
      ...
      destruct f.
      iDestruct (wp_unfold with "H") as "H". rewrite /wp_pre /= Hetov.
      iSpecialize ("H" $! (_,s0,f_locs,f_inst) 0 [] [] 0).
      ...
      destruct x0.
      iSpecialize ("H" $! x (_,ws',f_locs0,f_inst0) [] with "[]").
      { iPureIntro. split;auto. exact H2. }
  ...
  Lemma wp_seq_trap_ctx (s : stuckness) (E : coPset) (es1 es2 : language.expr wasm_lang) (f f': frame) i lh :
    ↪[frame] f ∗
     (↪[frame] f -∗ WP es1 @ NotStuck; E {{ w, ⌜w = trapV⌝ ∗ ↪[frame] f' }})
  ...
  Lemma wp_trap_ctx (s : stuckness) (E : coPset) (f: frame) i lh vs es :
    const_list vs ->
    ↪[frame] f -∗
  ...
  (* This is like wp_seq_can_trap_ctx' but it lets you include a postcondition for traps. *)
  Lemma wp_seq_can_trap_ctx_precise (s : stuckness) (E : coPset) (Φ : iris.val -> iProp Σ) (Ψ : frame -> iris.val -> iProp Σ) (es1 es2 : language.expr wasm_lang) (i : nat) (lh : lholed) (Φf : frame -> iProp Σ) (Φt: iProp Σ) :
    ⊢ (∀ f0, Ψ f0 trapV ={E}=∗ ⌜False⌝) -∗
      (∀ f, ↪[frame] f ∗ Φt ∗ Φf f -∗ Φ trapV) -∗
      WP es1 @ NotStuck; E {{ w, ∃ (f0: frame), (⌜w = trapV⌝ ∗ Φt ∨ Ψ f0 w) ∗ ↪[frame] f0 ∗ Φf f0 }} -∗
      (∀ w (f0: frame), 
        Ψ f0 w ∗ ↪[frame] f0 ∗ Φf f0 -∗ 
        WP (iris.of_val w ++ es2) @ s; E CTX i; lh {{ v, Φ v }}) -∗
      WP (es1 ++ es2) @ s; E CTX i; lh {{ v, Φ v }}.
  Proof.
    iLöb as "IH" forall (s E es1 es2 Φ Ψ i lh).
    {
      iIntros "Hntrap Ht Hes1 Hes2".
      iIntros (LI Hfilled).
      repeat rewrite wp_unfold /wp_pre /=.
      ...
      { iApply wp_unfold. rewrite /wp_pre /= Hetov.
        ...
        rewrite Hvs'.
        iMod "Hes1" as "(%f0 & Hes1 & Hf)".
        iDestruct "Hes1" as "[[-> Htrap] | Hes1]".
        { apply to_val_trap_is_singleton in Hvs' as ->.
        ...
        { iApply wp_unfold. rewrite /wp_pre /= Hetov.
          ...
          - iIntros (e2 σ2 efs HStep').
            ...
            + assert (iris.prim_step es1 σ [] [AI_trap] σ []) as HStep2.
              ...
              rewrite /wp_pre /=. iMod "Hes''" as "(%f0 & [[_ Htrap] | Hcontr] & Hf)".
  ...
  Lemma wp_seq_can_trap 
    (s : stuckness) (E : coPset)
    (Φ : iris.val -> iProp Σ) 
    (Ψ : frame -> iris.val -> iProp Σ)
    (Φf : frame -> iProp Σ)
    (Φt: iProp Σ)
    (es1 es2 : language.expr wasm_lang) :
    ⊢ (∀ f0, Ψ f0 trapV ={E}=∗ ⌜False⌝) -∗
      (∀ f, ↪[frame] f ∗ Φt ∗ Φf f -∗ Φ trapV) -∗
      WP es1 @ NotStuck; E {{ w, ∃ (f0: frame), (⌜w = trapV⌝ ∗ Φt ∨ Ψ f0 w) ∗ ↪[frame] f0 ∗ Φf f0 }} -∗
      (∀ w (f0: frame), 
        Ψ f0 w ∗ ↪[frame] f0 ∗ Φf f0 -∗ 
        WP (iris.of_val w ++ es2) @ s; E {{ v, Φ v }}) -∗
      WP (es1 ++ es2) @ s; E {{ v, Φ v }}.
  Proof.
      iIntros "HΨnotrap Hframetrap Hes1 Hes2".
      iApply wp_wasm_empty_ctx.
      iApply (wp_seq_can_trap_ctx_precise with "[$] [$] [$]").
  ...
  (* This is like wp_seq_can_trap_ctx but it does modus ponens to
  remove a ↪[frame] f premise that was only ever used to instantiate
  the hypothesis on es1. *)
  Lemma wp_seq_can_trap_ctx' (s : stuckness) (E : coPset) (Φ Ψ : iris.val -> iProp Σ) (es1 es2 : language.expr wasm_lang) (i : nat) (lh : lholed) (Φf : frame -> iProp Σ) :
    (((Ψ trapV ={E}=∗ ⌜False⌝)) ∗
       (∀ f, ↪[frame] f ∗ Φf f -∗ Φ trapV) ∗
       (WP es1 @ NotStuck; E {{ w, (⌜w = trapV⌝ ∨ Ψ w) ∗ ∃ (f0: frame), ↪[frame] f0 ∗ Φf f0 }}) ∗
       ∀ w (f0: frame),
         Ψ w ∗ ↪[frame] f0 ∗ Φf f0 -∗ 
         WP (iris.of_val w ++ es2) @ s; E CTX i; lh {{ v, Φ v }})%I
     ⊢ WP (es1 ++ es2) @ s; E CTX i; lh {{ v, Φ v }}.
  Proof.
    iIntros "[Hntrap [Ht [Hes1 Hes2]]]".
    iApply (wp_seq_can_trap_ctx_precise s E Φ (λ _, Ψ) _ _ _ _ Φf True
             with "[Hntrap] [Ht] [Hes1] [Hes2]"); try by iFrame.
    - iIntros (f) "(Hf & HT & HΦf)".
      iApply "Ht"; iFrame.
    - iApply (wp_wand with "[Hes1]").
      + iApply "Hes1".
      + iIntros (v) "[[Htrap | Hnotrap] Hf]"; by iFrame.
  Qed.
...
(* This rule is useful in particular for semantic type soundness, which allows traps *)
  Lemma wp_seq_can_trap_ctx (s : stuckness) (E : coPset) (Φ Ψ : iris.val -> iProp Σ) (es1 es2 : language.expr wasm_lang) (i : nat) (lh : lholed) (Φf : frame -> iProp Σ) (f: frame) :
    (((Ψ trapV ={E}=∗ ⌜False⌝)) ∗
       (∀ f, ↪[frame] f ∗ Φf f -∗ Φ trapV) ∗
       ↪[frame] f ∗
       (↪[frame] f -∗ WP es1 @ NotStuck; E {{ w, (⌜w = trapV⌝ ∨ Ψ w) ∗ ∃ (f0: frame), ↪[frame] f0 ∗ Φf f0 }}) ∗
       ∀ w (f0: frame), Ψ w ∗ ↪[frame] f0 ∗ Φf f0 -∗ WP (iris.of_val w ++ es2) @ s; E CTX i; lh {{ v, Φ v }})%I
     ⊢ WP (es1 ++ es2) @ s; E CTX i; lh {{ v, Φ v }}.
  Proof.
    iIntros "[Hntrap [Ht [Hf [Hes1 Hes2]]]]".
    iSpecialize ("Hes1" with "Hf").
  ...
  Lemma wp_val_can_trap_precise (s : stuckness) (E : coPset) (Φ : frame -> val -> iProp Σ) (v0 : value) (es : language.expr wasm_lang) Φf Φt :
    ((∀ f, Φ f trapV ={E}=∗ ⌜False⌝) ∗
       (WP es @ NotStuck ; E {{ v, ∃ f, ↪[frame] f ∗ Φf f ∗ (⌜v = trapV⌝ ∗ Φt ∨ (Φ f (val_combine (immV [v0]) v))) }})
       ⊢ WP ((AI_basic (BI_const v0)) :: es) @ s ; E {{ v, ∃ f, ↪[frame] f ∗ Φf f ∗ (⌜v = trapV⌝ ∗ Φt ∨ Φ f v) }})%I.
  Proof.
  ...
  { destruct vs.
    { iApply wp_unfold.
      ...
      iMod "H" as "(%f & Hf & HΦf & [[%Heq Htr]| HΦ])";try done.
      iModIntro. iFrame. }
    { apply to_val_trap_is_singleton in Hes as ->.
      repeat rewrite wp_unfold /wp_pre /=.
      iMod "H" as "(%f0 & Hf & HΦf & [[%Heq Htr]| Hcontr])";cycle 1.
      { iApply fupd_wp. iMod ("Hntrap" with "Hcontr") as "?". done. }
      iApply (wp_wand  _ _ _ (λ v, (⌜v = trapV⌝ ∗ Φt) ∗ ↪[frame] f0)%I with "[Hf Htr]").
      { rewrite -(take_drop 1 [AI_basic (BI_const v0); AI_trap]);simpl take;simpl drop.
        rewrite -(app_nil_r [AI_trap]).
        iApply (wp_trap with "[Htr] Hf");auto. }
      iIntros (v) "[[-> Htr] H]". iExists _; iFrame; iLeft; by iFrame. }
    { iApply wp_unfold.
      repeat rewrite wp_unfold /wp_pre /=.      
      rewrite Hes. apply of_to_val in Hes as <-; erewrite to_val_cons_brV;[|apply to_of_val].
      iMod "H" as "(%f0 & Hf & HΦf & [[%Heq Htr]| Htrap])"; try done.
      iModIntro. iFrame. }
    { iApply wp_unfold.
      repeat rewrite wp_unfold /wp_pre /=.      
  ...
  { iApply wp_unfold.
    ...
    - iIntros (es2 σ2 efs HStep).
      ...
      + move/lfilledP in Hlf1.
        ...
        iDestruct (wp_unfold with "Hes") as "Hes".
        rewrite /wp_pre /=.
        iMod "Hes" as "(%f0 & Hf & HΦf & [[%Heq Htr]| Htrap])"; cycle 1.
        { iApply fupd_wp. by iMod ("Hntrap" with "Htrap"). }
        iApply (wp_wand  _ _ _ (λ v, (⌜v = trapV⌝ ∗ Φt) ∗ ↪[frame] f0)%I with "[Hf Htr]").
        { rewrite separate1.
          iApply (wp_trap with "[Htr] Hf");auto. }
        iIntros (v) "[[-> Htr] H]". iExists _; iFrame; iLeft; by iFrame. }
  ...
  Proof.
    ...
    iApply (wp_val_can_trap_precise _ _ (λ f, Φ)).
    - iSplitL "Hntrap"; [by iFrame|]. 
      instantiate (1:= ⌜True⌝%I).
      iApply (wp_wand with "[H]").
      + iApply "H".
      + iIntros (v) "[[Htrap | Hpost] Hfr]"; by iFrame.
    - iIntros (v) "(%f0 & Hf0 & HΦf & [[Htrap HT] | HΦ])"; iFrame.
  Qed.
  ...
  Lemma wp_val_return' (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs vs' es' es'' n (f0: frame) Φf :
    const_list vs ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     (↪[frame] f0 -∗ ↪[RUN] -∗ WP vs' ++ vs ++ es'' @ s; E {{ v, Φ v ∗ ∃ f, ↪[frame] f ∗ Φf f }})
     -∗ WP vs @ s; E CTX 1; LH_rec vs' n es' (LH_base [] []) es'' {{ v, Φ v ∗ ∃ f, ↪[frame] f ∗ Φf f }}.
  Proof.
    iIntros (Hconst) "Hf0 Hrun HWP".
    iLöb as "IH".
    ...
    iApply (wp_seq _ _ _ (λ v, ((⌜v = immV (v2 ++ v1)⌝ ∗ ↪[RUN]) ∗ ↪[frame] f0))%I).
  ...
  Lemma wp_val_can_trap_app (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs v' (es : language.expr wasm_lang) (f: frame) Φf :
    iris.to_val vs = Some (immV v') ->
    (□ ((Φ trapV ={E}=∗ ⌜False⌝))) ∗ ↪[frame] f ∗
                     (↪[frame] f -∗ WP es @ NotStuck ; E {{ v, (⌜v = trapV⌝ ∨ Φ (val_combine (immV v') v)) ∗ ∃ (f: frame), ↪[frame] f ∗ Φf f }})%I
...
Lemma wp_seq_can_trap_same_ctx (Φ Ψ : iris.val -> iProp Σ) (s : stuckness) (E : coPset) (es1 es2 : language.expr wasm_lang) (f: frame) i lh :
...
Lemma wp_seq_can_trap_same_empty_ctx (Φ Ψ : iris.val -> iProp Σ) (s : stuckness) (E : coPset) (es1 es2 : language.expr wasm_lang) (f: frame) :
    (Ψ trapV ={E}=∗ False) ∗ (Φ trapV) ∗ ↪[frame] f ∗
    (↪[frame] f -∗ WP es1 @ NotStuck; E {{ w, (⌜w = trapV⌝ ∨ Ψ w) ∗ ↪[frame] f }}) ∗
