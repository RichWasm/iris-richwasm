...
Section control_rules.
  ...
  (* Control flow rules *)
  Lemma wp_return (s: stuckness) (E: coPset) (Φ: val -> iProp Σ) es vs vs0 n (f0 f:frame) i lh:
    iris.to_val vs = Some (immV vs0) ->
    length vs = n ->
    lfilled i lh (vs ++ [AI_basic BI_return]) es ->
    ↪[RUN] -∗
    ▷ WP vs @ s; E {{ v, Φ v ∗ ↪[frame] f }} -∗
                 WP [AI_local n f0 es] @ s; E {{ v, (Φ v ∗ ↪[RUN]) ∗ ↪[frame] f }}%I.
  Proof.
    iIntros (Hval Hlen Hlf) "Hrun HΦ".
...
(*      destruct σ as [[ [? ws] locs] inst]. *)
  ...
  Lemma wp_frame_return (s: stuckness) (E: coPset) (Φ: val -> iProp Σ) vs vs0 n (f0 f: frame) i lh LI:
    iris.to_val vs = Some (immV vs0) ->
    length vs = n ->
    lfilled i lh (vs ++ [AI_basic BI_return]) LI ->
    ( ↪[RUN] ∗ ▷ WP vs @ s; E {{ v, Φ v ∗ ↪[frame] f }}
                   ⊢ WP LI @ s; E FRAME n ; f0 {{ v, (Φ v ∗ ↪[RUN])  ∗ ↪[frame] f }}).
  Proof.
    iIntros (Hval Hlen Hlf) "[Hrun HΦ]".
    iApply (wp_return with "Hrun") => //.
  Qed.

  Lemma wp_ctx_frame_return (s: stuckness) (E: coPset) (Φ: iris.val -> iProp Σ) vs vs0 n (f0 f: frame) i lh :
    iris.to_val vs = Some (immV vs0) ->
    length vs = n ->
    (↪[RUN] ∗ ▷ WP vs @ s; E {{ v, Φ v ∗ ↪[frame] f }}
                   ⊢ WP vs ++ [AI_basic BI_return] @ s; E FRAME n ; f0 CTX i ; lh {{ v, (Φ v ∗ ↪[RUN]) ∗ ↪[frame] f }}).
  Proof.
    iIntros (Hval Hlen) "[Hrun HΦ]".
    iIntros (LI HLI).
    iApply (wp_return with "Hrun");eauto.
  Qed.

  Lemma wp_br (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n vs es i LI lh (f: frame) :
    const_list vs ->
    length vs = n ->
    lfilled i lh (vs ++ [::AI_basic (BI_br i)]) LI ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP (vs ++ es) @ s; E {{ v, Φ v }})
     -∗ WP [AI_label n es LI] @ s; E {{ v, Φ v }}.
  Proof.
    iIntros (Hvs Hlen Hfill) "Hf Hrun HΦ".
    iApply wp_lift_step => //=.
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_br_ctx_nested (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n vs es i j lh lh' lh'' vs' es' (f : frame) vs0' n0 es0 es0' :
    S i < j ->
    ...
    length vs = n ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP (vs' ++ (vs ++ es) ++ es') @ s; E CTX j - S i ; lh'' {{ Φ }})
     -∗ WP vs ++ [::AI_basic (BI_br i)] @ s; E CTX j ; lh' {{ Φ }}.
  Proof.
    iIntros (Hlt Hlayer Hminus Hvs Hlen) "Hf Hrun HΦ".
    iIntros (LI Hfill).
 ...
 destruct σ as [[[??]?]?].
  ...
  inversion Hrun; subst o.
    ...
    iApply fupd_frame_l.
    eassert (iris.prim_step LI (_,_,_,_) [] LI_r (_,_,_,_) []) as Hstep.
...
(*    destruct σ as [[ws locs] inst] => //=. *)
    ...
    eapply reduce_det in HStep as [Heq | [(?& ?& Hstart) |
  ...
  Lemma wp_block (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs es n m t1s t2s (f : frame) :
    const_list vs ->
    length vs = n ->
    length t1s = n ->
    length t2s = m ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP [::AI_label m [::] (vs ++ to_e_list es)] @ s; E {{ v, Φ v }})
     -∗ WP (vs ++ [::AI_basic (BI_block (Tf t1s t2s) es)]) @ s; E {{ v, Φ v }}.
  Proof.
    iIntros (Hvs Hlen1 Hlen2 Hlen3) "Hf Hrun HΦ".
    iApply wp_lift_step => //=.
...
(*      destruct σ as [[ws locs] inst].*)
  ...
  Lemma wp_label_value (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) es m ces v (f: frame) :
    iris.to_val es = Some (immV v) -> 
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ Φ (immV v) -∗ WP [::AI_label m ces es] @ s; E {{ v, (Φ v ∗ ↪[RUN]) ∗ ↪[frame] f }}.
  Proof.
    iIntros (Hval) "Hf0 Hrun HP".
    iApply wp_lift_atomic_step => //=.
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_label_trap (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) es m ctx (f0 : frame):
...
(*      destruct σ as [[ws locs] inst] => //=. *)
      ...
      { destruct H ; (try by inversion Heqes0) ;
        ...
        - inversion Heqes0 ; subst.
          ...
          destruct vs0 => //.
          destruct vs0 => //. 
(*          simple_filled H1 i lh bef aft n l'' l'''.
      ...
      { destruct lh ; last by false_assumption.
        destruct (const_list l0) ; last by false_assumption.
      ...
      found_intruse (AI_label n l2 l4) H3 Hxl1.
      inversion Habs.
  Qed.

  Lemma wp_val_return (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs vs' es' es'' n( f:frame) :
    const_list vs ->
    ↪[frame] f -∗ ↪[RUN] -∗
     (↪[frame] f -∗ ↪[RUN] -∗ WP vs' ++ vs ++ es'' @ s; E {{ v, Φ v }})
     -∗ WP vs @ s; E CTX 1; LH_rec vs' n es' (LH_base [] []) es'' {{ v, Φ v }}.
  Proof.
    iIntros (Hconst) "Hf Hrun HWP".
    iLöb as "IH".
    iIntros (LI Hfill%lfilled_Ind_Equivalent).
    inversion Hfill;subst.
    inversion H8;subst.
   ...
   eapply to_val_cat_inv in Hv1 as Hvv;[|apply Hv2].
    iApply (wp_seq _ _ _ (λ v, ((⌜v = immV (v2 ++ v1)⌝ ∗ ↪[RUN]) ∗ ↪[frame] f))%I).
    iSplitR; first by iIntros "([%H _] & ?)".
    iSplitR "HWP".
    - iApply wp_val_app; first by apply Hv2.
      iSplit; first by iIntros "!> ([%H _] & ?)".
  ...
  Lemma wp_block_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) (i : nat) (lh : lholed) vs t1s t2s es n m (f0: frame):
    const_list vs ->
    length vs = n ->
    length t1s = n ->
    length t2s = m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_label m [::] (vs ++ to_e_list es)] @ s; E CTX i; lh {{ Φ }})
     -∗ WP (vs ++ [::AI_basic (BI_block (Tf t1s t2s) es)]) @ s; E CTX i; lh {{ Φ }}.
  Proof.
    iIntros (Hconst Hn Hn' Hm) "Hf0 Hrun HWP".
    iIntros (LI Hfill).
...
(*    destruct σ as [[ws locs] inst] => //=. *)
    ...
    eapply reduce_det in H as [H | [[? Hstart] |
                                                    (i1 & i2 & i3 & Hstart & Hstart1 & Hstart2 & Hσ) ]];
  ...
  Lemma wp_block_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) (i : nat) (lh : lholed) vs t1s t2s es n m n1 f1 (f0 : frame):
    const_list vs ->
    length vs = n ->
    length t1s = n ->
    length t2s = m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_label m [::] (vs ++ to_e_list es)] @ s; E FRAME n1; f1 CTX i; lh {{ v, Φ v }})
...
(*    destruct σ as [[ws locs] inst] => //=.*)
  ...
  Lemma wp_br_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n vs es i lh vs' es' (f0: frame):
    const_list vs ->
    length vs = n ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP (vs' ++ vs ++ es ++ es') @ s; E {{ Φ }})
     -∗ WP vs ++ [::AI_basic (BI_br i)] @ s; E CTX S i; LH_rec vs' n es lh es' {{ Φ }}.
  Proof.
    iIntros (Hvs Hlen) "Hf0 Hrun HΦ".
    iIntros (LI Hfill).
    assert (iris.to_val LI = None) as Hnone.
    { apply lfilled_Ind_Equivalent in Hfill.
      inversion Hfill;simplify_eq.
...
(*    destruct σ as [[ws locs] inst] => //=. *)
  ...
  Lemma wp_br_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n vs es i lh vs' es' (f0: frame) n1 f1 :
    const_list vs ->
    length vs = n ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP (vs' ++ vs ++ es ++ es') @ s; E FRAME n1; f1 {{ v, Φ v }})
...
(*    destruct σ as [[ws locs] inst] => //=. *)
  ...
  Lemma wp_loop_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs es n m t1s t2s i lh (f0: frame):
    const_list vs ->
    length vs = n ->
    length t1s = n ->
    length t2s = m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_label n [::AI_basic (BI_loop (Tf t1s t2s) es)] (vs ++ to_e_list es)] @ s; E CTX i; lh {{ Φ }})
     -∗ WP vs ++ [::AI_basic (BI_loop (Tf t1s t2s) es)] @ s; E CTX i; lh {{ Φ }}.
  Proof.
    iIntros (Hvs Hn Hn' Hm) "Hf0 Hrun HP".
    iIntros (LI Hfill).
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_loop_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs es n m t1s t2s i lh (f0: frame) n1 f1 :
    const_list vs ->
    length vs = n ->
    length t1s = n ->
    length t2s = m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_label n [::AI_basic (BI_loop (Tf t1s t2s) es)] (vs ++ to_e_list es)] @ s; E FRAME n1; f1 CTX i; lh {{ v, Φ v }})
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_loop (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs es n m t1s t2s (f: frame):
    const_list vs ->
    length vs = n ->
    length t1s = n ->
    length t2s = m ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP [::AI_label n [::AI_basic (BI_loop (Tf t1s t2s) es)] (vs ++ to_e_list es)] @ s; E {{ w, Φ w }})
     -∗ WP vs ++ [::AI_basic (BI_loop (Tf t1s t2s) es)] @ s; E {{ w, Φ w }}.
  Proof.
    iIntros (Hvs Hn Hn' Hm) "Hf Hrun HP".
  ...
  Lemma wp_if_true_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n tf e1s e2s i lh (f0: frame):
    n ≠ Wasm_int.int_zero i32m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_block tf e1s)] @ s; E CTX i; lh {{ Φ }})
     -∗ WP [::AI_basic (BI_const (VAL_int32 n)); AI_basic (BI_if tf e1s e2s)] @ s; E CTX i; lh {{ Φ }}.
  Proof.
    iIntros (Hn) "Hf0 Hrun HP".
    iIntros (LI Hfill).
    eapply lfilled_swap in Hfill as Hfill'; destruct Hfill' as [LI' Hfill'].
    iApply wp_lift_step => //=.
    { destruct (iris.to_val LI) eqn:Hcontr;auto.
...
(*      destruct σ as [[ws locs] inst].*)
  ...
  Lemma wp_if_true_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n tf e1s e2s i lh (f0: frame) n1 f1 :
    n ≠ Wasm_int.int_zero i32m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_block tf e1s)] @ s; E FRAME n1; f1 CTX i; lh {{ v, Φ v }})
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_if_true (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) c tf e1s e2s (f: frame):
    c ≠ Wasm_int.int_zero i32m ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP [::AI_basic (BI_block tf e1s)] @ s; E {{ w, Φ w }})
  ...
  Lemma wp_if_false_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n tf e1s e2s i lh (f0 : frame):
    n = Wasm_int.int_zero i32m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_block tf e2s)] @ s; E CTX i; lh {{ Φ }})
     -∗ WP [::AI_basic (BI_const (VAL_int32 n)); AI_basic (BI_if tf e1s e2s)] @ s; E CTX i; lh {{ Φ }}.
  Proof.
    iIntros (Hn) "Hf0 Hrun HP".
    iIntros (LI Hfill).
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_if_false_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n tf e1s e2s i lh (f0: frame) n1 f1 :
    n = Wasm_int.int_zero i32m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_block tf e2s)] @ s; E FRAME n1; f1 CTX i; lh {{ v, Φ v }})
     -∗ WP [::AI_basic (BI_const (VAL_int32 n)); AI_basic (BI_if tf e1s e2s)] @ s; E FRAME n1; f1 CTX i; lh {{ v, Φ v }}.
  Proof.
    iIntros (Hn) "Hf0 Hrun HP".
    iIntros (LI Hfill).
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_if_false (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) c tf e1s e2s (f: frame):
    c = Wasm_int.int_zero i32m ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP [::AI_basic (BI_block tf e2s)] @ s; E {{ w, Φ w }})
     -∗ WP [::AI_basic (BI_const (VAL_int32 c)); AI_basic (BI_if tf e1s e2s)] @ s; E {{ w, Φ w }}.
  Proof.
    iIntros (?) "Hf Hrun HP".
  ...
  Lemma wp_br_if_true_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n i j lh (f0 : frame):
    n ≠ Wasm_int.int_zero i32m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br i)] @ s; E CTX j; lh {{ Φ }})
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_br_if_true_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n i j lh (f0: frame) n1 f1 :
    n ≠ Wasm_int.int_zero i32m ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br i)] @ s; E FRAME n1; f1 CTX j; lh {{ v, Φ v }})
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_br_if_true (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) c i (f: frame):
    c ≠ Wasm_int.int_zero i32m ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br i)] @ s; E {{ w, Φ w }})
  ...
  Lemma wp_br_if_false (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) c i (f: frame):
    c = Wasm_int.int_zero i32m ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ Φ (immV [])
     -∗ WP [::AI_basic (BI_const (VAL_int32 c)); AI_basic (BI_br_if i)] @ s; E {{ w, (Φ w ∗ ↪[RUN]) ∗ ↪[frame] f }}.
  Proof.
    iIntros (Hn) "Hf Hrun HΦ".
    iApply wp_lift_atomic_step => //=.
    iIntros (σ ns κ κs nt) "Hσ !>".
     destruct σ as [[[??]?]?].
  iDestruct "Hσ" as "(?&?&?&?&?&?&?&?&?&Hobs)".
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_br_table_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) iss c i j k lh (f0 : frame):
    ssrnat.leq (S (Wasm_int.nat_of_uint i32m c)) (length iss) ->
    List.nth_error iss (Wasm_int.nat_of_uint i32m c) = Some j ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br j)] @ s; E CTX k; lh {{ Φ }})
     -∗ WP [::AI_basic (BI_const (VAL_int32 c)); AI_basic (BI_br_table iss i)] @ s; E CTX k; lh {{ Φ }}.
  Proof.
    iIntros (Hiss Hj) "Hf0 Hrun HP".
    iIntros (LI Hfill).
    eapply lfilled_swap in Hfill as Hfill'; destruct Hfill' as [LI' Hfill'].
    iApply wp_lift_step => //=.
    { destruct (iris.to_val LI) eqn:Hcontr;auto.
...
(*      destruct σ as [[ws locs] inst]. *)
  ...
  Lemma wp_br_table_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) iss c i j k lh (f0 : frame) n1 f1 :
    ssrnat.leq (S (Wasm_int.nat_of_uint i32m c)) (length iss) ->
    List.nth_error iss (Wasm_int.nat_of_uint i32m c) = Some j ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br j)] @ s; E FRAME n1; f1 CTX k; lh {{ v, Φ v }})
...
(*      destruct σ as [[ws locs] inst].*)
  ...
  Lemma wp_br_table (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) iss c i j (f: frame):
    List.nth_error iss (Wasm_int.nat_of_uint i32m c) = Some j ->
    ↪[frame] f -∗ ↪[RUN] -∗
     ▷ (↪[frame] f -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br j)] @ s; E {{ w, Φ w }})
  ...
  Lemma wp_br_table_length_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) iss c i j lh (f0 : frame):
...
(*      destruct σ as [[ws locs] inst]. *)
    ...
    - (* destruct σ as [[ws locs] inst] => //=.*)
      ...
      iIntros "?"; by iApply ("HP" with "[$] [$]").
  Qed.
  Lemma wp_br_table_length_local_ctx (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) iss c i j lh (f0: frame) n1 f1 :
    ssrnat.leq (length iss) (Wasm_int.nat_of_uint i32m c) ->
    ↪[frame] f0 -∗ ↪[RUN] -∗
     ▷ (↪[frame] f0 -∗ ↪[RUN] -∗ WP [::AI_basic (BI_br i)] @ s; E FRAME n1; f1 CTX j; lh {{ v, Φ v }})
...
(*      destruct σ as [[ws locs] inst].*)
  ...
  Lemma wp_br_table_length (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) iss c i (f: frame):
  ...
  Lemma wp_br_ctx_shift (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs i lh n es vs1 es2 l n1 l0 l1 :
    ...
    length vs = n1 ->
    ↪[RUN] -∗
      (↪[RUN] -∗ WP vs ++ [AI_basic (BI_br i)] @ s; E CTX S i; LH_rec l n1 l0 lh l1 {{ Φ }}) -∗
                                                         WP vs ++ [AI_basic (BI_br (S i))] @ s; E CTX S (S i); LH_rec l n1 l0 (push_base lh n es vs1 es2) l1 {{ Φ }}.
  Proof.
    iIntros (Hlen Hconst) "Hrun Hwp".
    iIntros (LI Hfill%lfilled_Ind_Equivalent).
...
iSpecialize ("Hwp" with "[$] []").
    ...
    rewrite /wp_pre /= H.
    iSpecialize ("Hwp" $! (Run, s0, l2, i0) n0 κ κ' n' with "[$]").
(*     destruct σ1 as [[ [o ?] ?] ?]. *)

    assert (reduce Run s0
                   {| f_locs := l2; f_inst := i0 |}
                   (l ++ [AI_label (length vs) l0 LI0] ++ l1) Run s0
                   {| f_locs := l2; f_inst := i0 |}
                   (l ++ (vs ++ l0) ++ l1)).
    ...
    assert (reduce Run s0
                   {| f_locs := l2; f_inst := i0 |}
                   (l ++ [AI_label (length vs) l0 LIi] ++ l1) Run s0 
                   {| f_locs := l2; f_inst := i0 |}
                   (l ++ (vs ++ l0) ++ l1)).
    { eapply r_label with (k:=0) (lh:=LH_base l l1);cycle 1.
      apply lfilled_Ind_Equivalent. by constructor.
    ...
    iSpecialize ("Hwp" $! _ (_,s1,l3,i1) with "[]").
  ...
  Lemma wp_br_ctx_shift_inv (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) vs i lh n es vs1 es2 l n1 l0 l1 :
    ...
    const_list vs1 ->
    length vs = n1 ->
    ↪[RUN] -∗ (↪[RUN] -∗ WP vs ++ [AI_basic (BI_br (S i))] @ s; E CTX S (S i); LH_rec l n1 l0 (push_base lh n es vs1 es2) l1 {{ Φ }}) -∗
                                                                 WP vs ++ [AI_basic (BI_br i)] @ s; E CTX (S i); LH_rec l n1 l0 lh l1 {{ Φ }}.
  Proof.
    iIntros (Hconst Hconst' Hlen) "Hrun Hwp".
    iIntros (LI Hfill).
  ...
  inversion Hrun; subst o.
    ...
    iSpecialize ("Hwp" $! (Run, s0, l2, i0) k κ1 κ2 m with "[$]").
(*    destruct σ1 as [[ ? ?] ?]. *)
    ...
    assert (reduce Run s0 
                   {| f_locs := l2; f_inst := i0 |}
                   (l ++ [AI_label (length vs) l0 LI1] ++ l1) Run s0 
                   {| f_locs := l2; f_inst := i0 |}
                   (l ++ (vs ++ l0) ++ l1)).
    { eapply r_label with (k:=0) (lh:=LH_base l l1);cycle 1.
      apply lfilled_Ind_Equivalent. by constructor.
  ...
  Lemma wp_ret_shift (s : stuckness) (E : coPset) (Φ : val -> iProp Σ) n f i lh j lh' LI LI' vs :
    ...
    WP [AI_local n f LI] @ s; E {{ Φ }}) -∗
                                WP [AI_local n f LI'] @ s; E {{ Φ }}.
  Proof.
    iIntros (Hconst Hlen Hfill1 Hfill2) "Hrun Hwp".

    iApply wp_unfold.
    
    assert (iris.to_val [AI_local n f LI] = None).
  ...
  rewrite /wp_pre /= H.
    ...
    iSpecialize ("Hwp" $! (Run, s0, l, i0) k κ1 κ2 m with "[$]").


    assert (reduce Run s0 
                   {| f_locs := l; f_inst := i0 |}
                   ([AI_local n f LI]) Run s0
                   {| f_locs := l; f_inst := i0 |}
                   vs).
    ...
    all: try by (rewrite separate1 Hfirst0 in H3; inversion H3).
    inversion H3;simplify_eq.
    iSpecialize ("Hwp" $! _ (_,s1,l0,i1) with "[]").
    { iPureIntro. unfold prim_step. repeat split;eauto. }
    iFrame.
  Qed.
