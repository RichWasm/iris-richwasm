...
Context `{!wasmG Σ}.
  ...
  Lemma wp_frame_bind (s : stuckness) (E : coPset) (Φ : iris.val -> iProp Σ) n (f f0:frame) LI :
  ...
  Proof.
    ...
    { iApply wp_unfold.
      ...
      iSpecialize ("H" $! (_,_,_,_) 0 κ [] 0 with "[$H1 $H2 $H3 $H4 $H5 $H6 $Hff]").
    ...
    { iApply wp_unfold. unfold wp_pre. simpl. rewrite Hetov'.
      ...
      { iPureIntro. destruct s =>//.
        ...
        destruct σ' as [[[??]?]?].
        eexists x,[AI_local n {| f_locs := l0; f_inst := i0 |} e'],(_,_,l,i),efs.
  ...
  Proof.
    ...
    { iApply wp_unfold. rewrite /wp_pre /= Hetov.
      ...
      { erewrite of_to_val;[|eauto].
        ...
        iDestruct (wp_unfold with "H") as "H".
        rewrite /wp_pre /= Hetov.
        iDestruct ("H" $! (_,_,_,_) 0 _ [] 0 with "Hσ") as "H".
        iFrame. }
      { iIntros (σ ns κ κs nt) "Hσ".
        destruct σ as [[[??]?]?].
        iDestruct ("H" $! (_,_,_,_) 0 [] [] 0 with "Hσ") as "H".
