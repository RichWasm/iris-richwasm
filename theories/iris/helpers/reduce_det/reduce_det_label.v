...
Lemma label_det obs s f es obs' s' f' es' les les' k lh obs2 ws2 f2 es2 nnn:
  (∀ (f f2 f1 : frame) (es2 es1 es : seq.seq administrative_instruction),
    reduce obs s f es obs' s' f1 es1
    → reduce obs s f es obs2 ws2 f2 es2 → length_rec es < nnn → reduce_det_goal obs' s' f1 es1 obs2 ws2 f2 es2 es) ->
  reduce obs s f es obs' s' f' es' ->
  lfilled k lh es les ->
  lfilled k lh es' les' ->
  reduce obs s f les obs2 ws2 f2 es2 ->
  length_rec les < S nnn ->
  ((∀ (f f2 f1 : frame) (es2 es1 es : seq.seq administrative_instruction),
      reduce obs s f es obs' s' f1 es1
      → reduce obs s f es obs2 ws2 f2 es2 → length_rec es < nnn → reduce_det_goal obs' s' f1 es1 obs2 ws2 f2 es2 es)
   → reduce obs s f es obs2 ws2 f2 es2 → length_rec es < S nnn → reduce_det_goal obs' s' f' es' obs2 ws2 f2 es2 es) ->
  reduce_det_goal obs' s' f' les' obs2 ws2 f2 es2 les.
Proof.
  ...
  { destruct lh as [bef aft |] ; last by false_assumption.
    ...
    { destruct aft. { (*  If bef and aft are both empty, induction hypothesis 
      ...
      assert (reducible (es ++ ys) (obs, s, f_locs f, f_inst f)) as Hred.
      { exists [], (es' ++ ys), (obs', s', f_locs f', f_inst f'), [].
        repeat split => //=.
        apply (r_label (k:=0) (lh:=LH_base [] ys) (es:=es) (es':=es')) ;
          unfold lfilled, lfill => //=.
        destruct f ; destruct f' => //=. }
      assert (prim_step ((es ++ ys) ++ [y]) (obs, s, f_locs f, f_inst f)
                        [] es2 (obs2, ws2, f_locs f2, f_inst f2) []) as Hstep.
      { repeat split => //=. rewrite <- H. by destruct f ; destruct f2. }
      destruct (reduce_append _ _ _ _ _ _ _ Hred Hstep) as [[ Hes2y Htakestep ]|
                                                            (lh & lh' & Htrap &
                                                             Htrap' & Hσ)].
      { assert (reduce obs s f (es ++ ys) obs' s' f' (es' ++ ys)).
        { apply (r_label (k:=0) (lh:=LH_base [] ys) (es:=es) (es':=es')) ;
      ...
      assert (reduce obs s f (es ++ ys) obs' s' f' (es' ++ ys)) as Hles.
      { apply (r_label (k:=0) (lh:=LH_base [] ys) (es:=es) (es':=es')) => //=.
        unfold lfilled, lfill => //=.
        unfold lfilled, lfill => //=. }
    ...
    assert (reduce obs s f (bef ++ es ++ aft) obs' s' f' (bef ++ es' ++ aft)) as Hles.
    { apply (r_label (k:=0) (lh:=LH_base bef aft) (es:=es) (es':=es')) => //=.
      unfold lfilled, lfill, const_list ; by rewrite Hbef. 
      unfold lfilled, lfill, const_list ; by rewrite Hbef. }
    assert (reducible (bef ++ es ++ aft) (obs, s, f.(f_locs), f.(f_inst))) as Hred.
    { exists [], (bef ++ es' ++ aft), (obs', s', f_locs f', f_inst f'), [].
    ...
    assert (prim_step ([AI_basic (BI_const v)] ++ bef ++ es ++ aft)
                      (obs, s, f_locs f, f_inst f) [] es2
                      (obs2, ws2, f_locs f2, f_inst f2) []) as Hstep.
    ...
    destruct (reduce_ves _ _ _ _ _ _ _ Hred Hstep) as [[ Hves2 Hdropstep] |
                                                       ( lh & lh' & Htrap & Htrap' &
                                                         Hσ )].
    { assert (reduce obs s f (bef ++ es ++ aft) obs' s' f' (bef ++ es' ++ aft)).
      { apply (r_label (k:=0) (lh:=LH_base bef aft) (es:=es) (es':=es'))
    ...
    repeat split => //= ; try by eapply lfilled_implies_starts.
    inversion Hσ ; subst.
    destruct f ; destruct f2 ; simpl in H8 ; simpl in H7 ; by subst. }
  ...
  { unfold lfilled, lfill in H0 ; fold lfill in H0. rewrite <- Hbef in H0.
    ...
    { destruct aft. {
        ...
        inversion H1 ; subst.
        assert (reduce o s f l4 obs' s' f' l0).
        ...
        assert (reduce o s f l4 o' s'0 f'0 l).
      ...
      assert (reducible (AI_label n es0 l :: ys) (obs, s, f_locs f, f_inst f)) as Hred.
      { exists [], (AI_label n es0 l0 :: ys), (obs', s', f_locs f', f_inst f'), [].
        repeat split => //=.
      ...
      assert (prim_step ((AI_label n es0 l :: ys) ++ [y]) (obs, s, f_locs f, f_inst f)
                        [] es2 (obs2, ws2, f_locs f2, f_inst f2) []) as Hstep.
      { repeat split => //=. simpl in H ; rewrite <- H. by destruct f ; destruct f2. }
      destruct (reduce_append _ _ _ _ _ _ _ Hred Hstep) as [[ Hes2y Htakestep ]|
                                                            (lh0 & lh' & Htrap &
                                                             Htrap' & Hσ)].
      { assert (reduce obs s f (AI_label n es0 l :: ys) obs' s' f'
    ...
    unfold const_list in Hbef.
    simpl in Hbef. apply Logic.eq_sym, andb_true_iff in Hbef as [Ha Hbef].
    assert (reduce obs s f (bef ++ AI_label n es0 l :: aft) obs' s' f'
    ...
    assert (reducible (bef ++ AI_label n es0 l :: aft)
                      (obs, s, f.(f_locs), f.(f_inst))) as Hred.
    { exists [], (bef ++ AI_label n es0 l0 :: aft), (obs', s', f_locs f', f_inst f'), [].
      repeat split => //=. destruct f ; destruct f' => //=. } 
    destruct a ; try by inversion Ha.
    destruct b ; try by inversion Ha.
