...
Lemma unop_det v op t obs s f obs' s' f' es:
  reduce obs s f [AI_basic (BI_const v); AI_basic (BI_unop t op)] obs' s' f' es ->
  reduce_det_goal obs s f [AI_basic (BI_const (app_unop op v))] obs' s' f' es [AI_basic (BI_const v); AI_basic (BI_unop t op)].
Proof.
  move => Hred.
