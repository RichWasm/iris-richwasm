...
Lemma loop_det vs n m t1s t2s obs s f es obs' s' f' es' :
  const_list vs ->
  length vs = n ->
  length t1s = n ->
  length t2s = m ->
  reduce obs s f (vs ++ [AI_basic (BI_loop (Tf t1s t2s) es)]) obs' s' f' es' ->
  reduce_det_goal obs s f [AI_label n [AI_basic (BI_loop (Tf t1s t2s) es)] (vs ++ to_e_list es)] obs' s' f' es' (vs ++ [AI_basic (BI_loop (Tf t1s t2s) es)]).
Proof.
