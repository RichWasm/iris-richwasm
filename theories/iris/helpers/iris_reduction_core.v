...
Section reduction_core.
  ...
  Lemma reduction_core bef0 es0 aft0 bef1 es1 aft1 es0' es1' obs s f obs' s' f' obs0 s0 f0 :
    reduce obs s f es0 obs0 s0 f0 es0' ->
    reduce obs s f es1 obs' s' f' es1' ->
    const_list bef0 ->
    const_list bef1 ->
    ...
    (exists core bc0 ac0 bc1 ac1 core',
        const_list bc0 /\
          ...
          bef0 ++ bc0 = bef1 ++ bc1 /\
          ac0 ++ aft0 = ac1 ++ aft1 /\
          reduce obs s f core obs' s' f' core' /\
          bc1 ++ core' ++ ac1 = es1') \/
  ...
  Proof.
    ...
    cut (forall nnnn, length es1 < nnnn ->
                 (∃ core bc0 ac0 bc1 ac1 core' : seq.seq administrative_instruction,
                     ...
                     ∧ bef0 ++ bc0 = bef1 ++ bc1
                     ∧ ac0 ++ aft0 = ac1 ++ aft1
                     ∧ reduce obs s f core obs' s' f' core' ∧ bc1 ++ core' ++ ac1 = es1')
                 ∨ (∃ lh0 lh1 : lholed, lfilled 0 lh0 [AI_trap] es0 ∧ lfilled 0 lh1 [AI_trap] es1 /\ (obs,s,f) = (obs',s',f'))).
      ...
      - left ; repeat rewrite app_assoc in Heq.
        ...
        { destruct H ;
          ...
          - rewrite - (app_nil_r [_]) in Heq.
            ...
            eapply (block_not_enough_arguments_no_reduce _ s f vs0 t1s0 t2s0 es0
                                                        _ s f [AI_label m0 [] (vs0 ++ to_e_list es0)]) => //=.
    ...
    - left ; repeat rewrite app_assoc in Heq.
      ...
      + rewrite - (app_nil_r [_]) in Heq.
        apply first_values in Heq as (-> & Hblock & <-) ; try done ; try (by destruct He0 ; destruct e0 => // ; destruct b => //) ;
          try (destruct He0 as [-> | ->]; by intros [? ?]).
        eapply (invoke_not_enough_arguments_no_reduce_native _ s f _ _ _ s f) => //=.
        rewrite Hr2 in Hr1.
    ...
    - left ; repeat rewrite app_assoc in Heq.
      ...
      + rewrite - (app_nil_r [_]) in Heq.
        apply first_values in Heq as (-> & Hblock & <-) ; try done ; try (by destruct He0 ; destruct e0 => // ; destruct b => //) ; 
          try (destruct He0 as [-> | ->]; by intros [? ?]).
        eapply (invoke_not_enough_arguments_no_reduce_host _ s f _ _ _ s f) => //=.
        rewrite Hr2 in Hr1.
        inversion Hblock ; subst a.
        exact Hr1.
        by econstructor.
