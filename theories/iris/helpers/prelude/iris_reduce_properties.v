...
From RWasm.iris.language.iris Require Export iris.
From Wasm Require Export stdpp_aux.
From RWasm Require Export properties.
...
Section reduce_properties.
  ...
  Lemma values_no_reduce obs s f vs obs' s' f' es' :
    reduce obs s f vs obs' s' f' es' -> const_list vs -> False.
  Proof.
...
Ltac no_reduce Heqes Hred :=
  ...
  clear - (*host_function host function_closure store_record
                      host_instance*)
                      Hred Heqes ;
  induction Hred as [o o' e e' s f H | | | | | a | a | a | | | | | | | | | | | | | | |
                      obs s f es les obs' s' f' es' les' k lh  Hred IHreduce H0 _ |
    ]; (try by inversion Heqes) ; (try by found_intruse (AI_invoke a) Heqes Hxl1) ;
  [ destruct H as [ | | | | | | | | | | | | | | 
                    vs es n' m t1s' t2s Hconst Hvs Ht1s Ht2s |
                    vs es n' m t1s' t2s Hconst Hvs Ht1s Ht2s |
                  | | | | | | | | | | | | | 
                    es' lh obs Htrap' H0 ]; (try by inversion Heqes) ;
...
Section reduce_properties_lemmas.
  ...
  Lemma empty_no_reduce obs s f obs' s' f' es' :
    reduce obs s f [] obs' s' f' es' -> False.
  Proof.
    intro Hred.
  ...
  Lemma test_no_reduce1 obs s f v obs' s' f' es' :
    reduce obs s f [AI_basic (BI_const v)] obs' s' f' es' -> False.
  Proof.
    intro Hred.
  ...
  Lemma test_no_reduce_trap obs s f obs' s' f' es' :
    reduce obs s f [AI_trap] obs' s' f' es' -> False.
  Proof.
    intro Hred.
  ...
  Lemma test_no_reduce2 obs s f v1 v2 obs' s' f' es' :
    reduce obs s f [AI_basic (BI_const v1) ; AI_basic (BI_const v2)] obs' s' f' es' -> False.
  Proof.
    intro Hred.
  ...
  Lemma test_no_reduce3 obs s f v1 v2 v3 obs' s' f' es' :
    reduce obs s f [AI_basic (BI_const v1) ; AI_basic (BI_const v2) ; AI_basic (BI_const v3)]
           obs' s' f' es' -> False.
  Proof.
    intro Hred.
  ...
  Lemma reduce_load_false obs s0 f obs' s' f' es es' x0 x1 x2 x3 x4:
    es = [AI_basic (BI_load x0 x1 x2 x3 x4)] ->
    reduce obs s0 f es obs' s' f' es' -> False.
  Proof.
    intros Heq Hred.
  ...
  Lemma reduce_store_false obs s0 f obs' s' f' es es' x0 x1 x2 x3 x4:
  ...
  Lemma reduce_store_false_2 obs s0 f obs' s' f' es es' x0 x1 x2 x3 x4 v :
  ...
  Lemma reduce_val_false obs s0 f obs' s' f' es es' :
  ...
  Lemma reduce_br_false obs s f vs0 j es es0 obs' s' f' es' :
    const_list vs0 ->
    es = vs0 ++ [AI_basic (BI_br j)] ++ es0 ->
    reduce obs s f es obs' s' f' es' -> False.
  Proof.
    intros Heqes Hred.
  ...
  Lemma reduce_ret_false obs s f vs0 es es0 obs' s' f' es' :
    const_list vs0 ->
...
Ltac not_enough_arguments obs s f vs obj t1s obs' s' f' es' := 
  let Hxl1 := fresh "Hxl1" in
  let n := fresh "n" in
  let H := fresh "H" in
  let Hvs := fresh "Hvs" in
  ...
  cut (forall n, length vs < n ->
            reduce obs s f (vs ++ [obj]) obs' s' f' es'
  ...
  (try 
     (destruct H as [ | | | | | | | | | | | | | | 
                      ...
                      vs' es n' m t1s' t2s' Hconst' Hvs' Ht1s' Ht2s' |
                    | | | | | | | | | | | | i v | 
                      es' lh obs Htrap' H0 ]; try by found_intruse obj Heqes Hxl1 ;
...
Section reduce_properties_lemmas.
  ...
  Lemma block_not_enough_arguments_no_reduce obs s f (vs : seq.seq administrative_instruction)
        t1s t2s esb obs' s' f' es'  :
    reduce obs s f (vs ++ [AI_basic (BI_block (Tf t1s t2s) esb)]) obs' s' f' es' ->
    const_list vs ->
    length vs < length t1s ->
    False.
  Proof.
    not_enough_arguments obs s f vs (AI_basic (BI_block (Tf t1s t2s) esb)) t1s obs' s' f' es'.
  ...
  Lemma loop_not_enough_arguments_no_reduce obs s f (vs : seq.seq administrative_instruction)
        t1s t2s esb obs' s' f' es'  :
    reduce obs s f (vs ++ [AI_basic (BI_loop (Tf t1s t2s) esb)]) obs' s' f' es' ->
    const_list vs ->
    length vs < length t1s ->
    False.
  Proof.
    not_enough_arguments obs s f vs (AI_basic (BI_loop (Tf t1s t2s) esb)) t1s obs' s' f'
                         es'.
  ...
  Lemma invoke_not_enough_arguments_no_reduce_native
        obs s f vs a0 obs' s' f' es' i' t1s t2s ts es'' :
    nth_error (s_funcs s) a0 = Some (FC_func_native i' (Tf t1s t2s) ts es'') ->
    reduce obs s f (vs ++ [AI_invoke a0]) obs' s' f' es' ->
    const_list vs ->
    length vs < length t1s ->
    False.
  Proof.
    intro Hs.
  not_enough_arguments obs s f vs (AI_invoke a0) t1s obs' s' f' es'.
  - assert ([v ; AI_basic (BI_tee_local i)] = [v] ++ [AI_basic (BI_tee_local i)]).
...
(*  - apply app_inj_tail in Heqes. destruct Heqes as [Hves Hinvoke].
  ...
  Lemma invoke_not_enough_arguments_no_reduce_host
        obs s f vs a0 obs' s' f' es' t1s t2s h :
  ...
  Lemma AI_trap_irreducible obs ws f wobs' s' f' es':
    reduce obs ws f [AI_trap] wobs' s' f' es' ->
    False.
  Proof.
    move => HReduce.
  ...
  Lemma AI_trap_reduce_det v obs ws f obs' ws' f' es':
    reduce obs ws f ([AI_basic (BI_const v); AI_trap]) obs' ws' f' es' ->
  ...
  Lemma first_non_value_reduce obs s f es obs' s' f' es' :
    reduce obs s f es obs' s' f' es' ->
  ...
  Lemma trap_reduce_length obs s f es obs' s' f' es' vs1 es2 :
    lfilled 0 (LH_base vs1 es2) [AI_trap] es -> reduce obs s f es obs' s' f' es' ->
    exists n1 n2, (n1 + (length es2 - n2) < length vs1 + length es2 ∧ n1 <= length vs1 ∧ n2 <= length es2) ∧
               lfilled 0 (LH_base (take n1 vs1) (drop n2 es2)) [AI_trap] es' /\ (obs, s, f) = (obs', s', f').
  Proof.
  ...
  Lemma trap_reduce obs s f es obs' s' f' es' lh :
    lfilled 0 lh [AI_trap] es -> reduce obs s f es obs' s' f' es' ->
    exists lh', lfilled 0 lh' [AI_trap] es' /\ (obs, s, f) = (obs', s', f').
  Proof.
    intros Hfill Hred.
  ...
  Lemma local_frame_reducible n e o s v i v' i' :
    reducible e (o,s,v,i) ->
    reducible [AI_local n (Build_frame v i) e] (o,s,v',i').
  Proof.
    intros [obs [e' [σ' [efs Hred]]]].
    unfold reducible, language.reducible.
    destruct σ' as [[[? ?] ?] ?].
    exists [], [AI_local n (Build_frame l i0) e'], (o0, s0,v',i'), [].
    rewrite /= /iris.prim_step in Hred.
  ...
  Lemma local_frame_lfilled_reducible j lh LI n e o s v i v' i' :
    lfilled j lh e LI ->
    reducible e (o, s,v,i) ->
    reducible [AI_local n (Build_frame v i) LI] (o,s,v',i').
  Proof.
    intros Hfill Hred.
  ...
  Lemma reduce_focus obs s f es obs' s' f' es':
    reduce obs s f es obs' s' f' es' ->
    (exists k lh vs e es'', const_list vs /\ (is_const e = false) /\
                         reduce obs s f (vs ++ [e]) obs' s' f' es''  /\
  ...
  Lemma br_no_reduce obs s f lh i es obs' s' f' es' :
  ...
  Proof.
    ...
    - found_intruse (AI_basic (BI_br i)) Hfill Hxl1.
      apply in_app_or in Hxl1 as [Habs | Habs].
      assert (const_list ves) as Hconst ; last by intruse_among_values ves Habs Hconst.
      rewrite H1 ; by apply v_to_e_is_const_list. inversion Habs ; inversion H5.
  ...
  Lemma lfilled_first_non_value obs s f es obs' s' f' es' k lh les les':
    reduce obs s f es obs' s' f' es' ->
    lfilled k lh es les ->
    lfilled k lh es' les' ->
    exists vs e esf es' k0 lh0,
      const_list vs /\
        ...
        (is_const e = false ) /\ 
        reduce obs s f (vs ++ e :: esf) obs' s' f' es' /\
  ...
  Lemma reduce_return_trap_label es obs obs' s' f' vs n es'0 es'' es' :
    reduce obs s' f' es obs' s' f' es' ->
    const_list vs ->
  ...
  Proof.
    ...
    found_intruse (AI_label n es'0 [AI_trap]) Heq Hxl1.
    apply in_app_or in Hxl1 as [|[|]] => //.
    assert (const_list ves) as H' ; last by intruse_among_values ves H5 H'.
    ...
    { apply lfilled_Ind_Equivalent in H. inversion H;subst.
      apply const_list_snoc_eq3 in H1;auto.
      2: eapply properties.reduce_not_nil;eauto.
      2:{ destruct (const_list es) eqn:Habs => //.
          exfalso ; eapply values_no_reduce => //. }
  ...
  Lemma reduce_focus_one es1 obs s f obs' s' f' vs n es'0 LI es'' es' :
    reduce obs s f es1 obs' s' f' es' ->
    es1 = (vs ++ [AI_label n es'0 LI] ++ es'') ->
    const_list vs ->
    iris.to_val LI = None ->
    (∀ i j lh vs0, const_list vs0 -> lfilled i lh (vs0 ++ [AI_basic (BI_br j)]) LI -> False) ->
    ∃ LI', reduce obs s f LI obs' s' f' LI'.
  Proof.
    intros Hred.
    ...
    found_intruse (AI_label n' es'0 LI) Heq Hxl1.
    apply in_app_or in Hxl1 as [|[|]] => //.
    assert (const_list ves) as H' ; last by intruse_among_values ves H5 H'.
    by subst ; apply v_to_e_is_const_list.    
    { subst.
  ...
  Lemma trap_reduce_state_eq i lh es obs s f obs' s' f' es' :
    reduce obs s f es obs' s' f' es' ->
  ...
  Proof.
    ...
    [ found_intruse (AI_trap) H2 Hxl1
    | found_intruse (AI_label n es' LI) H2 Hxl1].
    all: try by subst ; intros i' lh Hfill%lfilled_Ind_Equivalent;
      inversion Hfill ; 
      subst ;
      apply Logic.eq_sym in H0 ;
    [ found_intruse (AI_trap) H0 Hxl1
    | found_intruse (AI_label n es' LI) H0 Hxl1].
    all: try by subst ; intros i' lh Hfill%lfilled_Ind_Equivalent;
      inversion Hfill ; 
      subst ;
      apply Logic.eq_sym in H3 ;
    [ found_intruse (AI_trap) H3 Hxl1
    | found_intruse (AI_label n es' LI) H3 Hxl1].
    all: try by subst ; intros i' lh Hfill%lfilled_Ind_Equivalent;
      inversion Hfill ; 
      subst ;
  ...
  Lemma trap_reduce_lfilled i lh es obs s f obs' s' f' es' :
    reduce obs s f es obs' s' f' es' ->
    lfilled i lh [AI_trap] es -> 
  ...
  Proof.
    ...
    apply Logic.eq_sym in H5. found_intruse AI_trap H5 Hxl1.
    apply in_app_or in Hxl1 as [|[|]] => //.
    assert (const_list ves) as Habs ; last by intruse_among_values ves H10 Habs.
    by subst ; apply v_to_e_is_const_list.
    apply Logic.eq_sym in H5. found_intruse (AI_label n0 es' LI) H5 Hxl1.
    apply in_app_or in Hxl1 as [|[|]] => //.
    assert (const_list ves) as Habs ; last by intruse_among_values ves H11 Habs.
    by subst ; apply v_to_e_is_const_list.    
    { subst.
      apply lfilled_Ind_Equivalent in H.
      inversion H.
  ...
  Lemma trap_reduce_nested obs s f es obs' s' f' es' lh i :
    lfilled i lh [AI_trap] es -> reduce obs s f es obs' s' f' es' ->
  ...
  Lemma trap_context_reduce obs locs s LI lh k :
    lfilled (S k) lh [AI_trap] LI ->
    ∃ e', reduce obs locs s LI obs locs s e'.
  Proof.
    revert LI lh.
  ...
  Proof.
    ...
    found_intruse (AI_label n es es1) Heqes'' Hxl1.
    apply in_app_or in Hxl1 as [|[|]] => //.
    assert (const_list (v_to_e_list vcs)) ; first by apply v_to_e_is_const_list.
    intruse_among_values (v_to_e_list vcs) H0 H1.
