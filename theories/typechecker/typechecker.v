                    ...
                    then
                      ...
                      | Some (κs, τs) =>
                          ...
                          then
                            ...
                            then has_instruction_type_ok_checker M F ψ L'
                            else INR "incorrect instruction type for case_load (failed looping check)"
                          else INR "incorrect instruction type for case_load (potentially copying mm refs)"
                      | None => INR "incorrect instruction type for case_load (τs_ser isn't all SerT)"
                      end
                    else INR "incorrect instruction type for case_load (input/output don't match)"
                | _ => INR "incorrect instruction type for case_load (wrong output shape)"
                end
            | _ => INR "incorrect instruction type for case_load (wrong input shape)"
            end
        | _ => INR "incorrect instruction type for case_load (wrong shape)"
        end
      else INR "incorrect instruction type for case_load"
  | IGroup ψ_inner =>
      if andb (instruction_type_beq ψ ψ_inner) (local_ctx_beq L L')
      then
        match ψ with
