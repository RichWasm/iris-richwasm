...
Definition extend {A} (f: positive -> A) (x : positive) (x' : A) : (positive -> A) :=
  fun z => if (z =? x)%positive then x' else f z.
...
Proof.
  intros Hinj Hin z Hinz. unfold compose. simpl.

  destruct (Pos.eqb_spec z x); subst; eauto.
  - unfold extend. rewrite !Pos.eqb_refl. reflexivity.
  - unfold extend. rewrite !(proj2 (Pos.eqb_neq _ _)); eauto.
Qed. 
...
Proof. 
  ...
  - destruct Hi as [x' [Hin Heq]]; subst. inv Hin.
    reflexivity.
  - destruct Hi. eexists; split; reflexivity.
Qed.
...
Proof. 
  ...
  intros x. unfold extend. destruct (Pos.eqb_spec x x2); eauto.
Qed.
...
Lemma extend_gso {A} f x (x' : A) y :
  ...
  f {x ~> x'} y = f y.
Proof. 
  intros Heq. unfold extend. rewrite (proj2 (Pos.eqb_neq _ _)); eauto.
Qed.
...
Lemma extend_same f x y :
  ...
  f {y ~> y} x = x. 
Proof.
  unfold extend. destruct (Pos.eqb_spec x y); eauto.
Qed.
...
Proof.
  intros Hnin Hnin' z. destruct (Pos.eqb_spec x z); eauto; subst.
  - rewrite extend_gss, extend_gso; eauto.
    now rewrite extend_gss.
  - destruct (Pos.eqb_spec x' z); eauto; subst.
...
Proof. 
  ...
  destruct (Pos.eqb_spec z x); eauto.
Qed.
...
Proof. 
  intros Heq x'.
    unfold extend. destruct (Pos.eqb_spec x' x); eauto.
    congruence. 
Qed.    
...
Proof. 
  ...
  destruct (Pos.eqb_spec z x). now eauto.
...
Proof.
  ...
  f_equal. eapply IHl.
  intros Hc; eapply Hnin; rewrite FromList_cons; eauto. now right.
  intros Hc; eapply Hnin; subst; rewrite FromList_cons. now left.
Qed.
...
Proof.
  ...
  - intros y [y' [Hin Heq]]; subst. 
    destruct (Pos.eqb_spec x y').
    + subst. rewrite extend_gss. now right.
    + rewrite extend_gso; eauto. left.
      eexists; split; eauto. constructor; eauto.
      intros Hc; inv Hc; congruence.
  - intros y Hin.
    destruct (Pos.eqb_spec x' y); subst.
    ...
    + inv Hin. 
      * destruct H as [y' [Hin Heq]]; subst. inv Hin.
        ...
        intros Heq; subst. eapply H0; reflexivity.
      * inv H. congruence. 
Qed.
...
Proof.  
  ...
  destruct (Pos.eqb_spec y' x); subst; [ now right |].
  left. eexists; eauto.
Qed.
...
Proof.
  ...
  -  destruct l'; try discriminate. rewrite FromList_cons in Hnin.
     destruct (Pos.eqb_spec x a); subst.
     + rewrite extend_gss.
...
Proof.
  ...
  now eauto with Ensembles_DB.
  intros Hc. eapply HD; eauto with Ensembles_DB.
Qed.
...
Proof.
  ...
  - rewrite extend_gss. intros Heq.
    destruct (Pos.eqb_spec y y'); [ now eauto | ].
...
Proof.
  ...
  destruct (Pos.eqb_spec x y); destruct (Pos.eqb_spec x z); subst; eauto;
...
Proof.
  ...
  unfold extend in *.
  edestruct (Pos.eqb_spec x1 x).
  - rewrite <- e in Heq.
    edestruct (Pos.eqb_spec x2 x1); [ now eauto |].
    exfalso. eapply Hin. eexists; eauto.
  - edestruct (Pos.eqb_spec x2 x).
    + exfalso. eapply Hin. eexists; eauto.
...
Proof. 
  ...
  subst. eapply Hinj in Heq'; eauto with Ensembles_DB.
  eapply H. subst; eauto with Ensembles_DB.
Qed.

Instance pos_eq_eqdec: EquivDec.EqDec positive eq.
Proof.
  intro; intros.
  destruct (Pos.eqb_spec x y); eauto.
Qed.
  ...
  - rewrite FromList_cons in Heq.
    ...
    + assert (Heq' := Heq).
      ...
      * rewrite <- image_Singleton.
        ...
        intros Hc; inv Hc; eauto with Ensembles_DB.
Qed.
...
Proof.
  intros y [z Hin].
  destruct (Pos.eqb_spec x y); subst; eauto with Ensembles_DB.
  right. eexists. rewrite extend_gso; eauto with Ensembles_DB.
Qed.
...
Proof.
  split.
  - intros z Hin. destruct (Pos.eqb_spec x z); subst.
    repeat eexists; eauto. rewrite extend_gss. reflexivity.
    inv Hin. inv H. congruence.
    edestruct H. 
    repeat eexists; eauto. rewrite extend_gso; eauto.
  - intros z Hin. destruct (Pos.eqb_spec x z); subst; eauto with Ensembles_DB.
    right. destruct Hin as [w H1]. rewrite extend_gso in H1; eauto with Ensembles_DB.
    repeat eexists; eauto with Ensembles_DB.
Qed.
...
Proof. 
  split; intros z H.
  - destruct H as [w H'].
    destruct (Pos.eqb_spec x z); subst; eauto with Ensembles_DB.
    rewrite extend_gso in H'; eauto. left.
    eexists; eauto with Ensembles_DB.
  - destruct (Pos.eqb_spec x z); subst; eauto.
...
Proof.
  ...
  - inv Hin. eexists; split; eauto with Ensembles_DB.
Qed. 
...
Proof.
  ...
  - inv Hin.
    + destruct H as [z [Hin Heq]].
      eexists; split; eauto with Ensembles_DB.
    + destruct H as [z [Hin Heq]].
      eexists; split; eauto with Ensembles_DB.
Qed.
...
Proof. 
  intros z [y [Heq Hin]].
  destruct (Pos.eqb_spec x y); subst.
  - rewrite extend_gss in Hin. inv Hin; eauto with Ensembles_DB.
...
Proof.
  ...
  - destruct Him as [z [y' [Heq Him]] | z Heq ].
    + eexists. split. now eapply Heq.
      ...
      subst. inv Heq; eauto with Ensembles_DB.
...
Proof.
  ...
  - destruct H' as [w [Hin Heq ]].
    inv Hin. rewrite Heq in H; inv H. eauto with Ensembles_DB.
  - inv H'. eexists; split; eauto with Ensembles_DB.
Qed.
...
Proof.
  intros z H'. 
  destruct H' as [w [Hin Heq ]].
  destruct (Pos.eqb_spec x w); subst.
...
Proof.
  ...
  - intros z H'. inv H'.
    + destruct H as [w [Hin' Heq]].
      destruct (Pos.eqb_spec x w); subst.
      * inv Hin'. exfalso; eauto with Ensembles_DB.
...
Proof.
  ...
  destruct (Pos.eqb_spec x z); subst. simpl.
  - rewrite !extend_gss. reflexivity.
