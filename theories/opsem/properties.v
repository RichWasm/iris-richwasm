...
Lemma r_eliml: forall obs s f es obs' s' f' es' lconst,
    const_list lconst ->
    reduce obs s f es obs' s' f' es' ->
    reduce obs s f (lconst ++ es) obs' s' f' (lconst ++ es').
Proof.
  move => obs s f es obs' s' f' es' lconst HConst H.
...
Lemma r_elimr: forall obs s f es obs' s' f' es' les,
    reduce obs s f es obs' s' f' es' ->
    reduce obs s f (es ++ les) obs' s' f' (es' ++ les).
Proof.
  move => obs s f es obs' s' f' es' les H.
  apply: r_label; try apply/lfilledP.
  - apply: H.
...
Lemma r_eliml_empty: forall obs s f es obs' s' f' lconst,
    const_list lconst ->
    reduce obs s f es obs' s' f' [::] ->
    reduce obs s f (lconst ++ es) obs' s' f' lconst.
Proof.
  move => obs s f es obs' s' f' lconst HConst H.
  assert (reduce obs s f (lconst++es) obs' s' f' (lconst++[::])); first by apply: r_eliml.
  by rewrite cats0 in H0.
Qed.

Lemma r_elimr_empty: forall obs s f es obs' s' f' les,
    reduce obs s f es obs' s' f' [::] ->
    reduce obs s f (es ++ les) obs' s' f' les.
Proof.
  move => obs s f es obs' s' f' les H.
  assert (reduce obs s f (es++les) obs' s' f' ([::] ++les)); first by apply: r_elimr.
...
Lemma not_reduce_simple_nil : forall o o' es', ~ reduce_simple o [::] o' es'.
Proof.
  assert (forall o o' es es', reduce_simple o es o' es' -> es = [::] -> False) as H.
  { move => o o' es es' H.
    elim: {o o' es es'} H => //=.
...
Lemma reduce_not_nil : forall obs σ1 f es obs' σ2 f' es',
  reduce obs σ1 f es obs' σ2 f' es' -> es <> [::].
Proof.
  move => obs σ1 f es obs' σ2 f' es' Hred.
