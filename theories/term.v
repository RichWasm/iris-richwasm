...
Definition mem_qual (mem : Memory) : Qual :=
  match mem with
  | Gc => Unrestricted
  | Manual => Linear
  end.

Definition loc_qual (loc_ctx : list Memory) (l : Loc) : option Qual :=
  match l with
  | LocV v => option_map mem_qual (List.nth_error loc_ctx v)
  | LocP p mem => Some (mem_qual mem)
  end.

Fixpoint type_qual (typ_ctx : list (Size * Qual * HeapableConstant))
                   (loc_ctx : list Memory)
                   (t : Typ)
                   : option Qual :=
  match t with
  | Num _ => Some (QualConst Unrestricted)
  | TVar v => option_map (fun '(_, q, _) => q) (List.nth_error typ_ctx v)
  | Unit => Some (QualConst Unrestricted)
  | ProdT x => None (* TODO *)
  | CoderefT x => Some (QualConst Unrestricted)
  | Rec q _ => Some q
  | PtrT _ => Some (QualConst Unrestricted)
  | ExLoc t => type_qual typ_ctx loc_ctx t
  | OwnR _ => Some (QualConst Linear)
  | CapT _ _ _ => Some (QualConst Linear)
  | RefT _ loc _ => loc_qual loc_ctx loc
  end.
...
(* Section TypInd.
  ...
  Fixpoint Typ_ind' (p : Typ) {struct p} : P p
  ...
  Proof.
    ...
    - destruct q. apply HQual, Typ_ind'.
  ...
  Proof.
    repeat split;
      (apply Typ_ind'||apply Typ_ind'||apply FunType_ind'
...
Section TypRect.
  ...
  Fixpoint Typ_rect' (p : Typ) {struct p} : P p
  ...
  Proof.
    ...
    - destruct q. apply HQual, Typ_rect'.
  ...
  Proof.
    repeat split;
      (apply Typ_rect'||apply Typ_rect'||apply FunType_rect'
