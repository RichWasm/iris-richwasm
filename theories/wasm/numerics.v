...
From Stdlib Require Import ZArith.Int ZArith.BinInt.
...
Fixpoint power_index_to_bits (c : nat) (l : seq Z) : seq bool :=
  match c with
  | 0 => [::]
  | S c => ((c : Z) \in l) :: power_index_to_bits c l
  end.
...
Proof.
  ...
  rewrite IH //. move=> i I. apply: A. lias. 
Qed.
...
Proof.
  move=> c l n /ssrnat.leP I. move: l. elim: I.
  - move=> l /=. by rewrite_by (S n - n - 1 = 0).
...
Definition canonical_pl := Zpower.shift_pos (Z.to_pos prec - 2) 1.
...
Proof.
  ...
  rewrite Zpower.shift_pos_correct Z.pow_pos_fold.
...
Proof.
  ...
  rewrite_by (Zaux.radix_val Zaux.radix2 = 2).
  rewrite_by (Z.pos precn - 1 - 1 = Z.pos (precn - 2)).
  split; first by lias.
  rewrite_by (Z.pos precn - 1 = 1 + Z.pos (precn - 2)).
  rewrite Zaux.Zpower_plus => //.
  rewrite_by (2 ^ 1 = 2).
...
Proof.
  ...
  - move: C => /= C. move: prec_gt_0. case Eprec: prec => [|precn|] // _.
    ...
    + rewrite_by (Z.pos (Pos.lor pl canonical_pl) = Z.lor (Z.pos pl) (Z.pos canonical_pl)).
      rewrite Z.log2_lor => //. rewrite Zpower.shift_pos_correct Z.pow_pos_fold.
...
Definition ZofB_param (divP divN : Z -> Z -> Z) (z : T) :=
  ...
  | Binary.B754_finite s m 0%Z _ =>
    Some (SpecFloat.cond_Zopp s (Z.pos m))
  | Binary.B754_finite s m (Z.pos e) _ =>
    Some (SpecFloat.cond_Zopp s (Z.pos m) * Z.pow_pos (Zaux.radix_val Zaux.radix2) e)%Z
  | Binary.B754_finite s m (Z.neg e) _ =>
    let: div := if s then divN else divP in
    Some (SpecFloat.cond_Zopp s (div (Z.pos m) (Z.pow_pos (Zaux.radix_val Zaux.radix2) e)))
  | _ => None
  end.
