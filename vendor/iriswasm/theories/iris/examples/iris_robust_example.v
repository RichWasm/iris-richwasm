...
From Wasm.iris.logrel Require Export iris_interp_instance_alloc.
      ...
      { iIntros (idnstart) "Hf [Hmod_lse Hr]".
        ...
        { (* Using the previously proved lse spec to resolve the reasoning *)
          ...
          { iSplitR "Hmem".  eauto.
            ...
            cbn.
            replace (repeat #00%byte (N.to_nat (page_size*1))) with (repeat #00%byte 4%nat ++ repeat #00%byte (Pos.to_nat 65532)).
