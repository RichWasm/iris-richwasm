...
Definition parse_memarg {n} : byte_parser (immediate * alignment_exponent * static_offset) n :=
  (extract_globalidx (fun x => x) <$> parse_globalidx) <&> parse_alignment_exponent <&> parse_static_offset.

Definition uncurry3 {A B C D} (f : A -> B -> C -> D) (x : (A * B * C)) :=
  let '(a, b, c) := x in
  f a b c.

Definition parse_i32_load {n} : byte_parser basic_instruction n :=
  exact_byte x28 &> (uncurry3 (fun x y z => BI_load x T_i32 None y z) <$> parse_memarg).  

Definition parse_i64_load {n} : byte_parser basic_instruction n :=
  exact_byte x29 &> (uncurry3 (fun x => BI_load x T_i64 None) <$> parse_memarg).

Definition parse_f32_load {n} : byte_parser basic_instruction n :=
  exact_byte x2a &> (uncurry3 (fun x => BI_load x T_f32 None) <$> parse_memarg).

Definition parse_f64_load {n} : byte_parser basic_instruction n :=
  exact_byte x2b &> (uncurry3 (fun x => BI_load x T_f64 None) <$> parse_memarg).

Definition parse_i32_load8_s {n} : byte_parser basic_instruction n :=
  exact_byte x2c &> (uncurry3 (fun x => BI_load x T_i32 (Some (Tp_i8, SX_S))) <$> parse_memarg).

Definition parse_i32_load8_u {n} : byte_parser basic_instruction n :=
  exact_byte x2d &> (uncurry3 (fun x => BI_load x T_i32 (Some (Tp_i8, SX_U))) <$> parse_memarg).

Definition parse_i32_load16_s {n} : byte_parser basic_instruction n :=
  exact_byte x2e &> (uncurry3 (fun x => BI_load x T_i32 (Some (Tp_i16, SX_S))) <$> parse_memarg).

Definition parse_i32_load16_u {n} : byte_parser basic_instruction n :=
  exact_byte x2f &> (uncurry3 (fun x => BI_load x T_i32 (Some (Tp_i16, SX_U))) <$> parse_memarg).

Definition parse_i64_load8_s {n} : byte_parser basic_instruction n :=
  exact_byte x30 &> (uncurry3 (fun x => BI_load x T_i64 (Some (Tp_i8, SX_S))) <$> parse_memarg).

Definition parse_i64_load8_u {n} : byte_parser basic_instruction n :=
  exact_byte x31 &> (uncurry3 (fun x => BI_load x T_i64 (Some (Tp_i8, SX_U))) <$> parse_memarg).

Definition parse_i64_load16_s {n} : byte_parser basic_instruction n :=
  exact_byte x32 &> (uncurry3 (fun x => BI_load x T_i64 (Some (Tp_i16, SX_S))) <$> parse_memarg).

Definition parse_i64_load16_u {n} : byte_parser basic_instruction n :=
  exact_byte x33 &> (uncurry3 (fun x => BI_load x T_i64 (Some (Tp_i16, SX_U))) <$> parse_memarg).

Definition parse_i64_load32_s {n} : byte_parser basic_instruction n :=
  exact_byte x34 &> (uncurry3 (fun x => BI_load x T_i64 (Some (Tp_i32, SX_S))) <$> parse_memarg).

Definition parse_i64_load32_u {n} : byte_parser basic_instruction n :=
  exact_byte x35 &> (uncurry3 (fun x => BI_load x T_i64 (Some (Tp_i32, SX_U))) <$> parse_memarg).

Definition parse_i32_store {n} : byte_parser basic_instruction n :=
  exact_byte x36 &> (uncurry3 (fun x => BI_store x T_i32 None) <$> parse_memarg).

Definition parse_i64_store {n} : byte_parser basic_instruction n :=
  exact_byte x37 &> (uncurry3 (fun x => BI_store x T_i64 None) <$> parse_memarg).

Definition parse_f32_store {n} : byte_parser basic_instruction n :=
  exact_byte x38 &> (uncurry3 (fun x => BI_store x T_f32 None) <$> parse_memarg).

Definition parse_f64_store {n} : byte_parser basic_instruction n :=
  exact_byte x39 &> (uncurry3 (fun x => BI_store x T_f64 None) <$> parse_memarg).

Definition parse_i32_store8 {n} : byte_parser basic_instruction n :=
  exact_byte x3a &> (uncurry3 (fun x => BI_store x T_i32 (Some Tp_i8)) <$> parse_memarg).

Definition parse_i32_store16 {n} : byte_parser basic_instruction n :=
  exact_byte x3b &> (uncurry3 (fun x => BI_store x T_i32 (Some Tp_i16)) <$> parse_memarg).

Definition parse_i64_store8 {n} : byte_parser basic_instruction n :=
  exact_byte x3c &> (uncurry3 (fun x => BI_store x T_i32 (Some Tp_i8)) <$> parse_memarg).

Definition parse_i64_store16 {n} : byte_parser basic_instruction n :=
  exact_byte x3d &> (uncurry3 (fun x => BI_store x T_i32 (Some Tp_i16)) <$> parse_memarg).

Definition parse_i64_store32 {n} : byte_parser basic_instruction n :=
  exact_byte x3e &> (uncurry3 (fun x => BI_store x T_i32 (Some Tp_i32)) <$> parse_memarg).

Definition parse_memory_size {n} : byte_parser basic_instruction n :=
  exact_byte x3f &> (exact_byte x00 &> (extract_globalidx BI_current_memory <$> parse_globalidx)).

Definition parse_memory_grow {n} : byte_parser basic_instruction n :=
  exact_byte x40 &> (exact_byte x00 &> (extract_globalidx BI_grow_memory <$> parse_globalidx)).
...
#[global] Instance tokBytes : Tokenizer byte := MkTokenizer (fun x => x).
