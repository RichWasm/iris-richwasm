...
From Stdlib Require Import Program Lia.
...
Proof.
  ...
  apply nth_error_ssr with (x0 := x) in H.
...
Proof with auto_rewrite_cond.
  ...
  induction d' => //=.
   { intros d Hd; destruct d; last lias.
    ...
    2:{ unfold be_size_list in Hbes.
        by destruct (List.fold_left _ _ _). }
    exists tm. unfold check in Hcheck. simpl in Hcheck. subst cts.
    split => //.
  ...
  - move => c cts bes.
    ...
    + rewrite be_size_list_rcons in Hs.
      ...
      apply le_le in Hd.
      assert (be_size_single e <= d') as Hmeasure; first by lias.
  ...
  - destruct e => //=; (try destruct f as [tn' tm']); auto_rewrite_cond; move => ? Hs Hct Hct2; simplify_type_update => //...
    ...
    + fold_remember_check.
      ...
      assert (be_size_list l0 <= d') as Hmeasure2; first by lias.
